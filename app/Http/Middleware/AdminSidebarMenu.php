...
class AdminSidebarMenu
{
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('supplier.view') || auth()->user()->can('supplier.view_own')) {
                            ...
                            if(count($business->subscriptions) > 0){
                                $pakcage_details = $business->subscriptions[count($business->subscriptions) - 1 ]["package_details"];
                                $is_credit_note = isset($pakcage_details["creditnote_module"]) && $pakcage_details["creditnote_module"] == 1 ? true : false;

                            if($is_credit_note){
                                $sub->url(
                                    action([\Modules\CreditNote\Http\Controllers\CreditNoteController::class, 'index'], ['type' => 'creditnote']),
                                    "Credit Note",
                                    ['icon' => '', 'active' => request()->input('type') == 'creditnote']
                                );  
                            }
                        }
                        }
    ...
    $sub->url(
        ...
        ['icon' => '', 'active' => request()->segment(1) == 'labels' && request()->segment(2) == 'enhanced-show']
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own'))) {
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseOrderController::class, 'listDrafts']),
                                __('lang_v1.purchase_order_drafts'),
                                ['icon' => '', 'active' => request()->segment(2) == 'drafts']
                            );
                        }
            ...
            //Schemes dropdown
            if (in_array('schemes', $enabled_modules) && (auth()->user()->can('scheme.view'))) {
                $menu->dropdown(
                    __('scheme::lang.schemes'),
                    function ($sub) {
                        $sub->url(
                            action([\Modules\Scheme\Http\Controllers\SchemeController::class, 'index']),
                            __('scheme::lang.all_schemes'),
                            ['icon' => '', 'active' => request()->segment(1) == 'schemes' && request()->segment(2) == null]
                        );

                        if (auth()->user()->can('scheme.create')) {
                            $sub->url(
                                action([\Modules\Scheme\Http\Controllers\SchemeController::class, 'create']),
                                __('scheme::lang.add_scheme'),
                                ['icon' => '', 'active' => request()->segment(1) == 'schemes' && request()->segment(2) == 'create']
                            );
                        }
                    },
                    ['icon' => ' <svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                || auth()->user()->can('expense_report.view') || (\App\System::getProperty('customermonthlysales_version') && auth()->user()->can('customermonthlysales.view'))) {
                $menu->dropdown(
                    __('report.reports'),
                    function ($sub) use ($enabled_modules, $is_admin) {
                        // Customer Monthly Sales Report
                        if (\App\System::getProperty('customermonthlysales_version') && auth()->user()->can('customermonthlysales.view')) {
                            $sub->url(
                                route('reports.customer-monthly-sales'),
                                __('Customer Monthly Sales'),
                                ['icon' => '', 'active' => request()->segment(1) == 'reports' && request()->segment(2) == 'customer-monthly-sales']
                            );
                        }
                        ...
                        if (auth()->user()->can('stock_report.view')) {
                            ...
                            if(\App\Business::where('id', auth()->user()->business_id)->get()[0]["enable_lot_number"]){
                                $sub->url(
                                     route("get_lot_history"),
                                    __('Identifier History'),
                            ...
                            if (in_array('stock_adjustment', $enabled_modules)) {
                                $sub->url(
                                    ...
                                    ['icon' => '', 'active' => request()->segment(2) == 'get_lot_history']
                                );
                            }
                        }
        ...
        $module_data = $moduleUtil->getModuleData('modifyAdminMenu');
