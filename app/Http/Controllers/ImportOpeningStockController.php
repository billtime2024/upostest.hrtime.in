...
class ImportOpeningStockController extends Controller
{
        ...
        try {
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    //Read IMEI and Serial numbers
                    $imei_numbers = !empty($value[6]) ? array_filter(array_map('trim', explode("\n", $value[6]))) : [];
                    $serial_numbers = !empty($value[7]) ? array_filter(array_map('trim', explode("\n", $value[7]))) : [];

                    //Validate IMEI/Serial numbers
                    if ($product_info->item_type == 'imei') {
                        if (count($imei_numbers) != trim($value[2])) {
                            $is_valid = false;
                            $error_msg = "IMEI count does not match quantity in row no. $row_no";
                            break;
                        }
                        foreach ($imei_numbers as $imei) {
                            if (!preg_match('/^\d{15}$/', $imei)) {
                                $is_valid = false;
                                $error_msg = "Invalid IMEI format '$imei' in row no. $row_no. Must be 15 digits.";
                                break 2;
                            }
                            $existing = ImeiNumber::where('identifier', $imei)->first();
                            if ($existing) {
                                $is_valid = false;
                                $error_msg = "IMEI '$imei' already exists in row no. $row_no";
                                break 2;
                            }
                        }
                    } elseif ($product_info->item_type == 'serial') {
                        if (count($serial_numbers) != trim($value[2])) {
                            $is_valid = false;
                            $error_msg = "Serial count does not match quantity in row no. $row_no";
                            break;
                        }
                        foreach ($serial_numbers as $serial) {
                            ...
                            if ($existing) {
                                $is_valid = false;
                                $error_msg = "Serial '$serial' already exists in row no. $row_no";
                                break 2;
                            }
                        }
                    }
                    ...
                    $this->addOpeningStock($opening_stock, $product_info, $business_id, $unit_cost_before_tax, $os_transaction, $imei_numbers, $serial_numbers);
     ...
     * @param  array  $imei_numbers
     * @param  array  $serial_numbers
     * @return void
     */
    private function addOpeningStock($opening_stock, $product, $business_id, $unit_cost_before_tax, $transaction = null, $imei_numbers = [], $serial_numbers = [])
    {
        $user_id = request()->session()->get('user.id');
        ...
        if ($product->item_type == 'imei' && !empty($imei_numbers)) {
            foreach ($imei_numbers as $imei) {
                ImeiNumber::create([
        ...
        } elseif ($product->item_type == 'serial' && !empty($serial_numbers)) {
            foreach ($serial_numbers as $serial) {
                ImeiNumber::create([
                    'purchase_item_id' => $purchase_line->id,
