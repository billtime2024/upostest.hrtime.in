...
class OpeningStockController extends Controller
{
    ...
    public function add($product_id)
    {
        ...
        if (! empty($product) && $product->enable_stock == 1) {
            ...
            $transactions = Transaction::where('business_id', $business_id)
                                ...
                                ->with(['purchase_lines.imei_numbers'])
                                ->get();
            ...
            foreach ($transactions as $transaction) {
                foreach ($transaction->purchase_lines as $purchase_line) {
                    ...
                    $imei_numbers = $purchase_line->imei_numbers->pluck('identifier')->toArray();
                    if ($product->item_type == 'imei') {
                        $purchase_lines[$purchase_line->variation_id][$k]['imei_numbers'] = $imei_numbers;
                    } elseif ($product->item_type == 'serial') {
                        $purchase_lines[$purchase_line->variation_id][$k]['serial_numbers'] = $imei_numbers;
                    }
                }
            }
                    ...
                    if (array_key_exists($location_id, $locations)) {
                        foreach ($value as $vid => $purchase_lines_data) {
                            ...
                            foreach ($purchase_lines_data as $k => $pl) {
                                ...
                                $serial_numbers = [];
                                if ($product->item_type == 'imei') {
                                    $imei_numbers = array_filter(array_map('trim', explode("\n", $pl['imei_numbers'] ?? '')));
                                    if (count($imei_numbers) != $qty_remaining) {
                                        throw new \Exception(__('lang_v1.quantity_mismatch_for_imei_serial'));
                                    }
                                    foreach ($imei_numbers as $imei) {
                                        if (!preg_match('/^\d{15}$/', $imei)) {
                                            throw new \Exception("Invalid IMEI format: $imei");
                                        }
                                        $existing = ImeiNumber::where('identifier', $imei)->where('business_id', $business_id)->first();
                                        if ($existing && (!isset($pl['purchase_line_id']) || $existing->purchase_item_id != $pl['purchase_line_id'])) {
                                            throw new \Exception("IMEI $imei already exists");
                                        }
                                    }
                                } elseif ($product->item_type == 'serial') {
                                    $serial_numbers = array_filter(array_map('trim', explode("\n", $pl['serial_numbers'] ?? '')));
                                    if (count($serial_numbers) != $qty_remaining) {
                                        throw new \Exception(__('lang_v1.quantity_mismatch_for_imei_serial'));
                                    }
                                    foreach ($serial_numbers as $serial) {
                                        $existing = ImeiNumber::where('identifier', $serial)->where('business_id', $business_id)->first();
                                        if ($existing && (!isset($pl['purchase_line_id']) || $existing->purchase_item_id != $pl['purchase_line_id'])) {
                                            throw new \Exception("Serial $serial already exists");
                                        }
                                    }
                                }
                        ...
                        if (! empty($edit_purchase_lines)) {
                            foreach ($edit_purchase_lines as $t_id => $purchase_lines_data) {
                                $purchase_total = 0;
                                $updated_purchase_line_ids = [];
                                foreach ($purchase_lines_data as $data) {
                                    $purchase_line = $data['purchase_line'];
                                    $purchase_total += $purchase_line->purchase_price_inc_tax * $purchase_line->quantity;
                                    $updated_purchase_line_ids[] = $purchase_line->id;

                                    // Update IMEI/Serial numbers
                                    $this->updateImeiSerialNumbers($purchase_line, $data['imei_numbers'], $data['serial_numbers'], $location_id, $business_id);
                                }
                                ...
                                if ($delete_purchase_lines->count()) {
                                    foreach ($delete_purchase_lines as $delete_purchase_line) {
                                        ...
                                        // Delete IMEI/Serial numbers if not sold
                                        ImeiNumber::where('purchase_item_id', $delete_purchase_line->id)
                                            ->where('is_sold', 0)
                                            ->delete();
                                    }
                        ...
                        if (! empty($new_purchase_lines)) {
                            foreach ($new_purchase_lines as $key => $data) {
                                $new_purchase_line = $data['purchase_line'];
                                ...
                                // Create IMEI/Serial numbers
                                $this->updateImeiSerialNumbers($new_purchase_line, $data['imei_numbers'], $data['serial_numbers'], $location_id, $business_id);
    ...
    /**
     * Update IMEI/Serial numbers for a purchase line
     */
    private function updateImeiSerialNumbers($purchase_line, $imei_numbers, $serial_numbers, $location_id, $business_id)
    {
        $existing_imeis = ImeiNumber::where('purchase_item_id', $purchase_line->id)->get();
        $existing_identifiers = $existing_imeis->pluck('identifier')->toArray();

        $new_identifiers = array_merge($imei_numbers, $serial_numbers);

        // IMEIs to add
        $to_add = array_diff($new_identifiers, $existing_identifiers);
        // IMEIs to remove
        $to_remove = array_diff($existing_identifiers, $new_identifiers);

        // Add new ones
        foreach ($to_add as $identifier) {
            $type = in_array($identifier, $imei_numbers) ? ImeiNumber::TYPE_IMEI : ImeiNumber::TYPE_SERIAL;
            ImeiNumber::create([
                'purchase_item_id' => $purchase_line->id,
                'type' => $type,
        ...
        // Remove old ones if not sold
        foreach ($to_remove as $identifier) {
            $imei = $existing_imeis->where('identifier', $identifier)->first();
            if ($imei && !$imei->is_sold) {
                $imei->delete();
            }
        }
    }
