...
class LoginController extends Controller
{
    ...
    */
    ...
    public function logout_from_other_device($found_user){
         DB::delete("DELETE FROM `sessions` WHERE id=" . "'" . $found_user . "'");
         return redirect()->back()->with(
              'status',
                  ['success' => 1, 'msg' => 'you are logged out from other device']
             );
    }
    ...
    protected function authenticated(Request $request, $user)
    {
        ...
        $found_user = DB::select("select * from sessions where user_id=" . $user->id);
        
        if(count($found_user)  >  0) {
            //dd($found_user);
                 \Auth::logout();
            return view("auth.log_out_from_other_device", compact("found_user"));
           // DB::delete("DELETE FROM `sessions` WHERE id=" . "'" . $found_user[0]->id . "'");
            return redirect('/login')
              ->with(
                  'status',
                  ['success' => 0, 'msg' => 'you are already logged in  on other device']
                );
        }
