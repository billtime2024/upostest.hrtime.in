...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id', 'brand_id', 'unit_id', 'tax_id', 'type',
                'only_mfg_products', 'active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state', 'stock_filter', "repair_model_id", "product_list_filter_tax_id", "show_lot_number"]);

            $filters['not_for_selling'] = isset($filters['not_for_selling']) && $filters['not_for_selling'] == 'true' ? 1 : 0;
            $filters['show_lot_number'] = isset($filters['show_lot_number'])  ? $filters['show_lot_number']  : 0;
            $filters['show_manufacturing_data'] = $show_manufacturing_data;

            //Return the details in ajax call
            $for = request()->input('for') == 'view_product' ? 'view_product' : 'datatables';

            $products = $this->productUtil->getProductStockDetailsForMultipleSelectionFilter($business_id, $filters, $for);
            ...
            $datatable = Datatables::of($products)
                 ->addColumn("imei_serial", function ($row) use ($business_id){
                      $location_id = $row->location_id;
                      $variation_id = $row->variation_id;
                      $imei_serials = ImeiNumber::where('is_sold', false)->where('business_id', $business_id)->where('location_id', $location_id)->whereHas('purchaseLine', function($query) use ($variation_id) { $query->where('variation_id', $variation_id); })->pluck('identifier')->toArray();
                      return implode(", ", $imei_serials);
                  })
                 ->addColumn("lot_number", function ($row) use ($filters){
                     // dd($row);
                     if($filters["show_lot_number"] == 1){
                         $purchase_lines = \App\PurchaseLine::join('transactions as t', 'purchase_lines.transaction_id', "=", "t.id")
                         ->where("t.location_id", $row->location_id)
                         ->where("purchase_lines.product_id", $row->product_id)
                         ->where("purchase_lines.variation_id", $row->variation_id)
                         ...
                         // Aggregate purchase lines by lot number
                         $lot_number_data = [];
                         foreach($purchase_lines as $purchase_line) {
                             $lot_number = $purchase_line->lot_number;
                             if(!isset($lot_number_data[$lot_number])) {
                                 $lot_number_data[$lot_number] = [
                                     'total_quantity' => 0,
                                     'total_sold' => 0,
                                     'total_adjusted' => 0,
                                     'total_returned' => 0
                             ...
                             $lot_number_data[$lot_number]['total_quantity'] += (float) $purchase_line->quantity;
                             $lot_number_data[$lot_number]['total_sold'] += (float) $purchase_line->quantity_sold;
                             $lot_number_data[$lot_number]['total_adjusted'] += (float) $purchase_line->quantity_adjusted;
                             $lot_number_data[$lot_number]['total_returned'] += (float) $purchase_line->quantity_returned;
                         }

                         $result = "";
                         foreach($lot_number_data as $lot_number => $data) {
                             $available_quantity = $data['total_quantity'] - ($data['total_sold'] + $data['total_adjusted'] + $data['total_returned']);
    ...
    public function getStockDetails(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = Product::leftjoin('units as u', 'products.unit_id', '=', 'u.id')
                ...
                ->leftjoin('business_locations', 'vld.location_id', '=', 'business_locations.id')
    ...
    public function getExpenseReport(Request $request)
    {
        ...
        $dataset = $chart->dataset(__('report.total_expense'), 'pie', $values);
        $dataset->color(collect(['#7d5fff','#32ff7e', '#ff4d4d', '#7158e2','#3ae374', '#ff3838']));
        $chart->options(['pie' => [
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->leftjoin("users as rw", 'rw.id', '=', 't.res_waiter_id')
                ->where('t.business_id', $business_id)
                ->where('t.type', 'sell')
                ...
                ->select(
                    ...
                    DB::raw('CONCAT(COALESCE(rw.first_name, ""), COALESCE(rw.last_name, "")) as waiter')
                )
        ...
        if($request->get("location_id") === null){
            $request->merge(['location_id' => [ 0 => null]]);
        }
            $query->where(function ($query) use ($request){
                if (count($request->get('location_id')) > 0) {
                    foreach ($request->get('location_id') as $key => $location_id) {
                        if($location_id !== null){
                          if($key === 1){
                              $query->where('t.location_id', $location_id);
                          }else{
                              $query->orWhere('t.location_id', $location_id);
                          }
                       }
                    }
                }else {
            ...
            $request->merge(['category_id' => [ 0 => null]]);
        }
            $query->where(function ($query) use ($request){
                    ...
                    foreach ($request->get('category_id') as $key => $category_id) {
                        if($category_id !== null){
                          if($key === 1){
                              ...
                              $query->orWhere('p.category_id', $category_id);
                          }
                       }
                    }
                }else {
        ...
        if($request->get("sub_category_id") === null){
            $request->merge(['sub_category_id' => [ 0 => null]]);
        }

            $query->where(function ($query) use ($request){
                if (count($request->get('sub_category_id')) > 0) {
                    foreach ($request->get('sub_category_id') as $key => $sub_category_id) {
                        if($sub_category_id !== null){
                          if($key === 1){
                                $query->where('p.sub_category_id', $sub_category_id);
                          }else{
                              $query->orWhere('p.sub_category_id', $sub_category_id);
                          }
                       }
                    }
                }else {
                    $query->where('p.sub_category_id', '*');
                }
            });

             if($request->get("brand_id") === null){
            $request->merge(['brand_id' => [ 0 => null]]);
        }
            $query->where(function ($query) use ($request){
                if (count($request->get('brand_id')) > 0) {
                    foreach ($request->get('brand_id') as $key => $brand_id) {
                        if($brand_id !== null){
                          if($key === 1){
                                $query->where('p.brand_id', $brand_id);
                          }else{
                              $query->orWhere('p.brand_id', $brand_id);
                          }
                       }
                    }
                }else {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('purchase_price', function ($row){
                    if($row->purchase_price  !== null){
                        if(auth()->user()->can("view_purchase_price")){
                            ...
                            return  '<span class="display_currency" data-currency_symbol = true>'. (float) "0.00" .'</span>';
                        }
                    } else {
                        $new_purchase_line = TransactionSellLine::where('transaction_sell_lines.transaction_id', $row->transaction_id)
                                                ->join(
                                                        'transaction_sell_lines_purchase_lines as tsl',
                                                        'tsl.sell_line_id',
                                                        '=',
                                                        'transaction_sell_lines.id'
                                                    )
                                                ->join(
                                                        ...
                                                        'pl.id', 
                                                        '=',
                                                        'tsl.purchase_line_id'
                                                    )
                                                ->get();

                        if(count($new_purchase_line) > 0) {
                            return '<span class="display_currency" data-currency_symbol = true>'.$new_purchase_line[0]["purchase_price_inc_tax"] .'</span>';
                        } else {
                             return  '<span class="display_currency" data-currency_symbol = true>'. (float) "0.00" .'</span>';
                        }
                    }
                ...
                ->editColumn('service_staff', function ($row){
                    if(strlen($row->service_staff) > 0 ) {
                        return $row->service_staff;
                    }else if(strlen($row->waiter) > 0) {
                        return $row->waiter;
                    }else {
                        return "";
                    }
                ...
                ->rawColumns(['invoice_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax', 'customer', 'payment_methods', 'purchase_price'])
                ->make(true);
        }
        ...
        return view('report.product_sell_report')
            ->with(compact('business_locations', 'customers', 'categories', 'sub_categories', 'brands',
    ...
    public function getLotReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Product::where('products.business_id', $business_id)
                ...
                ->join('product_variations', 'product_variations.id', '=', 'v.product_variation_id')
                ->leftJoin(
                    'transaction_sell_lines_purchase_lines as tspl',
                    'pl.id',
                    '=',
      ...
      if($request->get("category_id") === null){
                    ...
                    foreach ($request->get('category_id') as $key => $category_id) {
                        if($category_id !== null){
                          if($key === 1){
                              ...
                              $query->orWhere('products.category_id', $category_id);
                          }
                       }
                    }
                }else {
        ...
        if($request->get("sub_category_id") === null){
                    ...
                    foreach ($request->get('sub_category_id') as $key => $sub_category_id) {
                        if($sub_category_id !== null){
                          if($key === 1){
                              ...
                              $query->orWhere('products.sub_category_id', $sub_category_id);
                          }
                       }
                    }
                }else {
                    ...
                    foreach ($request->get('brand_id') as $key => $brand_id) {
                        if($brand_id !== null){
                          if($key === 1){
                              ...
                              $query->orWhere('products.brand_id', $brand_id);
                          }
                       }
                    }
                }else {
            ...
            // }
                    ...
                    if($request->get("unit_id") === null){
            $request->merge(['unit_id' => [ 0 => null]]);
        }
            $query->where(function ($query) use ($request){
                if (count($request->get('unit_id')) > 0) {
                    foreach ($request->get('unit_id') as $key => $unit_id) {
                        if($unit_id !== null){
                          if($key === 1){
                                $query->where('products.unit_id', $unit_id);
                          }else{
                              $query->orWhere('products.unit_id', $unit_id);
                          }
                       }
                    }
                }else {
                    $query->where('products.unit_id', '*');
                }
            });
            ...
            // Apply stock filter
            $stock_filter = $request->input('stock_filter', 'exclude_zero_stock');
            $stock_column = DB::raw("(COALESCE(
    (
        SELECT 
            SUM(pls.quantity - pls.quantity_returned) 
        FROM 
            purchase_lines as pls 
        LEFT JOIN 
            transactions as t2 ON pls.transaction_id = t2.id 
        WHERE 
            pls.variation_id = v.id 
            AND pls.lot_number = pl.lot_number 
            AND t2.location_id = bl.id -- Specific location or permitted locations
    ), 0) 
    - SUM(COALESCE((tspl.quantity - tspl.qty_returned), 0))
            ...
            $query->select(
                'products.name as product',
                'v.name as variation_name',
                'product_variations.name as variation_template_name',
                'sub_sku',
                'bl.name as location',
                'pl.lot_number',
            ...
            // Apply the stock filter condition to the query
            if ($stock_filter == 'exclude_zero_stock') {
            ...
            ->editColumn('variation_name', function($row){
                if ($row->variation_name != 'DUMMY') {
                    return $row->variation_template_name . ' - ' . $row->variation_name;
                }else {
                    return '';
                }
              ...
              ->editColumn('created_at', function ($row){
                ...
                ->addColumn('stock_age', function($row){
                    return  $row->created_at->diffInDays(Carbon::now()) . ' Days';
                })
                  ->editColumn('purchase_price_inc_tax', function ($row){
                     return $row->currency . '<span data-is_quantity="true" class="display_currency total_purchased" data-currency_symbol="true" data-orig-value="' . number_format((float) $row->purchase_price_inc_tax, 2, '.', '') . '" >' . number_format((float) $row->purchase_price_inc_tax, 2, '.', '') . '</span></br>' ;
                    // return $row->currency . ' ' . number_format((float) $row->purchase_price_inc_tax, 2, '.', ''); 
                })
                ->editColumn('stock', function ($row) {
                    $stock = $row->stock ? $row->stock : 0;
                ...
                ->rawColumns(['exp_date', 'stock', 'total_sold', 'total_adjusted', 'purchase_price_inc_tax'])
                ->make(true);
        }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ->editColumn("supplier", function ($row){
                    if($row->supplier === "") {
                       $transaction_payment = \App\TransactionPayment::find($row->DT_RowId);
                       $supplier = \App\Contact::find($transaction_payment->payment_for);
                        return $supplier->supplier_business_name;
                    } else {
                        return $row->supplier;
                    }
                })
    ...
    public function getproductSellGroupedReport(Request $request)
    {
        ...
        if (count($request->get('location_id')) > 0) {
            foreach ($request->get('location_id') as $key => $location_id) {
                if(count($request->get('location_id')) > 1) {
                    if($location_id !== null){
                        $vld_str .= " OR vld.location_id=$location_id";
                    }
                }else{
                    if($location_id !== null){
                        $vld_str = "AND vld.location_id=$location_id";
                    }else {
                        // $vld_str = "AND vld.location_id=*";
                    }
                }
            }
        }else {
        ...
        //if($request->get("customer_group_id") === null){
        //    $request->merge(['customer_group_id' => [ 0 => null]]);
        //}
            //$query->where(function ($query) use ($request){
            //    if (count($request->get('customer_group_id')) > 0) {
            //        foreach ($request->get('customer_group_id') as $key => $customer_group_id) {
            ...
            //                    $query->leftjoin('contacts AS c', 't.contact_id', '=', 'c.id')
            //                        ->leftjoin('customer_groups AS CG', 'c.customer_group_id', '=', 'CG.id');
            //                    //->orWhere('CG.id', $customer_group_id);
            //                  //$query->orWhere('CG.id', $customer_group_id);
            //              }
            //           }
            //        }
            //    }else {
    ...
    public function getProfit($by = null)
    {
        ...
        // Get tax rate (default 18% as specified in the requirements)
        $tax_rate = request()->input('tax_rate', 18);
        ...
        $query->select(DB::raw('SUM(IF (TSPL.id IS NULL AND P.type="combo", (
            ...
            SELECT Sum((tspl2.quantity - tspl2.qty_returned) * (tsl.unit_price_inc_tax - pl2.purchase_price_inc_tax)) AS total
                ...
                ON tspl2.purchase_line_id = pl2.id
                WHERE tsl.parent_sell_line_id = transaction_sell_lines.id), IF(P.enable_stock=0,(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax,
                (TSPL.quantity - TSPL.qty_returned) * (transaction_sell_lines.unit_price_inc_tax - PL.purchase_price_inc_tax)) )) / (1 + (' . $tax_rate . ' / 100)) AS net_profit'),

                DB::raw('SUM(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) AS total_quantity'),

                DB::raw('SUM(transaction_sell_lines.unit_price_inc_tax * (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned)) / NULLIF(SUM(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned), 0) AS avg_selling_price')

            );
            // dd($query->toSql(), $business_id);
        ...
        if($by == 'product' || $by == 'category' || $by == 'brand' || $by == 'location' ) {
            $query->addSelect('V.id as variation_id');
            $query->addSelect('transaction_sell_lines.unit_price_inc_tax as selling_price');
            $query->addSelect('PL.purchase_price_inc_tax as purchase_price');
            $query->addSelect("P.unit_id");
        }
        ...
        if (in_array($by, ['invoice'])) {
            $datatable->editColumn('gross_profit', function ($row) {
                ...
                if($profit >= 0) {
                    ...
                    $html = '<span style="background-color: #e55a5a; color:#fff; padding: 0.25rem"  class="gross-profit" data-orig-value="'.$profit.'" >'.$this->transactionUtil->num_f($profit, true).'</span>';
                }
                ...
                function ($row) {
                    ...
                    if($row->gross_profit >= 0){
                        return '<span class="gross-profit" data-orig-value="'. $row->gross_profit. '">'.$this->transactionUtil->num_f($row->gross_profit, true).'</span>';
                         ...
                         return '<span class="gross-profit" style="background-color: #e55a5a; color:#fff; padding: 0.25rem" data-orig-value="'.$row->gross_profit.'">'.$this->transactionUtil->num_f($row->gross_profit, true).'</span>';
                    }
                });
        }
        ...
        if ($by == 'product' || $by == 'category' || $by == 'brand') {
            $request = request();
            $datatable->addColumn(
                "total_unit_sold",
                function ($row) use ($by)  {

                    if($row->unit_id && $by == 'product'){
                      $unit = \App\Unit::where("id", $row->unit_id)->first();
                    } else {
                        $unit = null;
                    }
                    $total_quantity = $row->total_quantity ?? 0;

                    $formatted_quantity = number_format($total_quantity, 2);

                      if(!empty($unit)){
                          return $formatted_quantity . " " . $unit->short_name ;
                      } else {
            ...
            $datatable->addColumn(
                ...
                function ($row) {
                    ...
                    if($total_quantity == 0) {
                        return $this->transactionUtil->num_f(0, true);
                    }

                    $avg_profit = $row->gross_profit / $total_quantity;

                    if($avg_profit >= 0){
                        return  $this->transactionUtil->num_f($avg_profit, true) ;
                    }else {
                        return  "<span style='background-color: #e55a5a; color: #fff; padding: 0.25rem;'>" . $this->transactionUtil->num_f($avg_profit, true) . "</span>";
                    }
                }
            );
            $raw_columns[] = 'avg_profit';
            ...
            $datatable->addColumn(
                ...
                function ($row) {
                    if($row->net_profit >= 0){
                        return '<span class="net-profit" data-orig-value="'. $row->net_profit. '">'.$this->transactionUtil->num_f($row->net_profit, true).'</span>';
                    } else {
                        return '<span class="net-profit" style="background-color: #e55a5a; color:#fff; padding: 0.25rem" data-orig-value="'.$row->net_profit.'">'.$this->transactionUtil->num_f($row->net_profit, true).'</span>';
                    }
                }
            );
            $raw_columns[] = 'net_profit';
            
            // Add Avg Net Profit/Unit column
            $datatable->addColumn(
                "avg_net_profit",
                function ($row) {
                    $total_quantity = $row->total_quantity ?? 0;
                    ...
                    $avg_net_profit = $row->net_profit / $total_quantity;

                    if($avg_net_profit >= 0){
                        return  $this->transactionUtil->num_f($avg_net_profit, true) ;
                    }else {
                        return  "<span style='background-color: #e55a5a; color: #fff; padding: 0.25rem;'>" . $this->transactionUtil->num_f($avg_net_profit, true) . "</span>";
                    }
                }
            );
            $raw_columns[] = 'avg_net_profit';
        }
    ...
    if($by == 'product' || $by == 'category' || $by == 'brand' || $by == 'location' ) {
        ...
        $datatable->addColumn(
            ...
            function ($row) {
                $total_quantity = $row->total_quantity ?? 0;
                $avg_selling_price = $row->avg_selling_price ?? 0;

                // Prevent division by zero
                if($total_quantity == 0 || $avg_selling_price == 0) {
                    return "0.00 %";
                }

                $gross_profit_percentage = (($row->gross_profit / $total_quantity ) / $avg_selling_price ) * 100;

                return round($gross_profit_percentage, 2). " % " ;
            }
        );
        ...
        $datatable->addColumn(
            ...
            function ($row) {
                ...
                $net_profit_percentage = (($row->net_profit / $total_quantity ) / $avg_selling_price ) * 100;

                return round($net_profit_percentage, 2). " % " ;
            }
        );
        $raw_columns[] = 'net_profit_percentage';
    }
        if($by == 'service_staff'){
    ...
    public function getStockValue()
    {
        ...
        $location_id = request()->input('location_id');
        $filters = request()->only(['category_id', 'sub_category_id', 'brand_id', 'unit_id',  'location_id']);
        ...
        $potential_profit = $closing_stock_by_sp - $closing_stock_by_pp;
            ...
            $profit_margin = empty($closing_stock_by_sp) ? 0 : $closing_stock_by_sp * 100;
        }else{
        ...
        return [
            ...
            'closing_stock_by_pp_round' => $this->changeToIndianStyle($closing_stock_by_pp),
            'closing_stock_by_sp_round' => $this->changeToIndianStyle($closing_stock_by_sp),
            'potential_profit_round' => $this->changeToIndianStyle($potential_profit),
            'profit_margin_round' => $profit_margin,
        ];
    }
    public function changeToIndianStyle ($inital_number)
    {
    ...
    public function gstSalesReport(Request $request)
    {
            ...
            return $datatable->addColumn('taxable_value', function ($row) {
               ...
               ->editColumn('short_code', function ($row) {
                    if(!empty($row->short_code)){
                        ...
                        return $row->product_custom_field4;
                    }
                })
                 ->editColumn('invoice_no', function ($row) {
                     return '<a data-href="'.action([\App\Http\Controllers\SellController::class, 'show'], [$row->transaction_id])
        ...
        return view('report.gst_sales_report')->with(compact('customers', 'taxes', 'business_locations'));
    }
    ...
    public function gstPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = PurchaseLine::join(
                ...
                ->whereNotNull('purchase_lines.tax_id')
                ->select(
                    'c.name as supplier',
                    'c.supplier_business_name',
                    'c.contact_id',
        ...
        return view('report.gst_purchase_report')->with(compact('suppliers', 'taxes', 'business_locations'));
    }
}
