...
class ImportProductsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            \Log::info('ImportProductsController store method started');
            ...
            if ($request->hasFile('products_csv')) {
                ...
                \Log::info('Excel parsed successfully, number of rows: ' . count($parsed_array[0]));
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    // Reward point enable & amount
                    $reward_enable = trim($value[37] ?? '');
                    if (!empty($reward_enable) && !in_array($reward_enable, ['0', '1'])) {
                        $is_valid = false;
                        $error_msg = 'Row ' . $row_no . ': Invalid REWARD POINT ENABLE (must be 0/1)';
                        break;
                    }
                    $product_array['enable_reward_points'] = !empty($reward_enable) ? (bool)$reward_enable : false;

                    $reward_amount = trim($value[38] ?? '');
                    if (!empty($reward_amount) && (!is_numeric($reward_amount) || $reward_amount < 0)) {
                        $is_valid = false;
                        $error_msg = 'Row ' . $row_no . ': Invalid REWARD POINT AMOUNT (numeric ≥0)';
                        break;
                    }
                    $product_array['reward_points_per_amount'] = !empty($reward_amount) ? (float)$reward_amount : 0;
                    ...
                    //Item Type
                    $item_type_value = trim($value[24]);
                    if ($item_type_value == '1') {
                        $product_array['item_type'] = 'imei';
                    } elseif ($item_type_value == '2') {
                        $product_array['item_type'] = 'serial';
                    } elseif ($item_type_value == '0' || empty($item_type_value)) {
                        $product_array['item_type'] = 'none';
                    } else {
                        $is_valid = false;
                        $error_msg = "Invalid value for ITEM TYPE in row no. $row_no. Must be 1 (imei), 2 (serial), or 0/blank (none)";
                        break;
                    }
                    ...
                    if ($product_array['type'] == 'single') {
                        ...
                        if (! empty($value[21]) && $enable_stock == 1) {
                            ...
                            if (! empty($value[23])) {
                                try {
                                    \Log::info('Parsing expiry date for row ' . $row_no . ': ' . trim($value[23]));
                                    $product_array['opening_stock_details']['exp_date'] = \Carbon::createFromFormat('m-d-Y', trim($value[23]))->format('Y-m-d');
                                    \Log::info('Expiry date parsed successfully for row ' . $row_no . ': ' . $product_array['opening_stock_details']['exp_date']);
                                } catch (\Exception $e) {
                                    \Log::info('Expiry date parse failed for row ' . $row_no . ': ' . $e->getMessage());
                                    $is_valid = false;
                                    $error_msg = "Invalid expiry date format in row no. $row_no. Expected format: mm-dd-yyyy";
                                    break;
                                }
                            } else {
                        ...
                        if (! empty($value[21]) && $enable_stock == 1) {
                            ...
                            foreach ($variation_os as $k => $v) {
                                ...
                                if (! empty($value[23])) {
                                    try {
                                        \Log::info('Parsing expiry date for variable row ' . $row_no . ' variation ' . $k . ': ' . trim($value[23]));
                                        $product_array['variation']['variations'][$k]['opening_stock_exp_date'] = \Carbon::createFromFormat('m-d-Y', trim($value[23]))->format('Y-m-d');
                                        \Log::info('Expiry date parsed successfully for variable row ' . $row_no . ' variation ' . $k . ': ' . $product_array['variation']['variations'][$k]['opening_stock_exp_date']);
                                    } catch (\Exception $e) {
                                        \Log::info('Expiry date parse failed for variable row ' . $row_no . ' variation ' . $k . ': ' . $e->getMessage());
                                        $is_valid = false;
                                        $error_msg = "Invalid expiry date format in row no. $row_no. Expected format: mm-dd-yyyy";
                                        break;
                                    }
    ...
    private function addOpeningStock($opening_stock, $product, $business_id)
    {
        ...
        \Log::info('financial_year.start value: ' . $transaction_date);
