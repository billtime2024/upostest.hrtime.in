...
{
    private static $moduleInstallCache = [];
    ...
    {
        if (isset(self::$moduleInstallCache[$module_name])) {
            return self::$moduleInstallCache[$module_name];
        }

        $is_available = Module::has($module_name);
        $caller = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 2)[1] ?? null;
        $caller_info = $caller ? ($caller['class'] ?? 'unknown') . '::' . ($caller['function'] ?? 'unknown') : 'unknown';
        if ($is_available) {
            //Check if installed by checking the system table {module_name}_version
            $version_key = strtolower($module_name).'_version';
            $module_version = System::getProperty($version_key);
            // dd($module_version, $module_name);
            if (empty($module_version)) {
                self::$moduleInstallCache[$module_name] = false;
                return false;
            } else {
                self::$moduleInstallCache[$module_name] = true;
                return true;
            }
        }
        self::$moduleInstallCache[$module_name] = false;
    ...
    public function hasThePermissionInSubscription($business_id, $permission, $callback_function = null)
    {
        if ($this->isSuperadminInstalled()) {
            ...
            if (empty($package)) {
                ...
                $result = $package['package_details'][$permission];
    ...
    public function availableModules()
    {
        return [
            ...
            'schemes' => ['name' => __('scheme::lang.schemes')],
