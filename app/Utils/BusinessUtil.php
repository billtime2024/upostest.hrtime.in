...
class BusinessUtil extends Util
{
    ...
    public function createNewBusiness($business_details)
    {
        ...
        $business_details['default_profit_percent'] = 5;
        ...
        $business_details['enable_inline_tax'] = 0;
            $custom_labels = [];
            $custom_labels["product"]["custom_field_4"]  = "HSN";
            $custom_labels["user"]["custom_field_1"]  = "WhatsApp API";
            
            $sms_settings["url"] = "https://app.watime.in/api/send";
            $sms_settings["send_to_param_name"] = "number";
            $sms_settings["msg_param_name"] = "message";
            $sms_settings["request_method"] = "GET";
            $sms_settings["param_1"] = "type";
            $sms_settings["param_val_1"] = "text";
            $sms_settings["param_2"] = "instance_id";
            $sms_settings["param_3"] = "access_token";
            $tax_label_1 = "GST:";
            $sms_settings["param_val_2"] = "";
            $sms_settings["param_val_3"] = "";
        $business_details["custom_labels"] = json_encode($custom_labels);
        $business_details["sms_settings"] = $sms_settings;
        $business_details["tax_label_1"] = $tax_label_1;
    ...
    public function sendWhatsapp($data)
    {
        ...
        if (empty($whatsapp_settings['instance_id']) || empty($whatsapp_settings['access_token'])) {
            return false;
        }

        $client = new \GuzzleHttp\Client();

        // Build query parameters as per API documentation
        $queryParams = [
        ...
        if (isset($data['media_url'])) {
            $queryParams['media_url'] = $data['media_url'];
        }

        if (isset($data['filename'])) {
            $queryParams['filename'] = $data['filename'];
        }

        try {
            $response = $client->get('https://app.watime.in/api/send', [
                'query' => $queryParams
            ]);
