...
class Util
{
     ...
     * @param  bool  $indian_format = false; If true, use Indian numbering system
     * @return string
     */
    public function num_f($input_number, $add_symbol = false, $business_details = null, $is_quantity = false, $indian_format = false)
    {
        ...
        if ($indian_format) {
            $formatted = $this->num_f_indian($input_number, $currency_precision, $decimal_separator);
        } else {
    ...
    /**
     * Formats number using Indian numbering system
     *
     * @param  float  $number
     * @param  int  $precision
     ...
     * @return string
     */
    public function num_f_indian($number, $precision = 2, $decimal_separator = '.')
    {
        ...
        // Format integer part using Indian system
        $formatted_integer = $this->formatIndianNumber($integer_part);

        // Add decimal part if exists
        if ($precision > 0 && !empty($decimal_part)) {
            // Pad decimal part with zeros if needed
            $decimal_part = str_pad($decimal_part, $precision, '0', STR_PAD_RIGHT);
            $formatted = $formatted_integer . $decimal_separator . $decimal_part;
        } else {
    ...
    /**
     * Helper function to format integer part using Indian numbering system
     *
     * @param  string  $number
     * @return string
     */
    private function formatIndianNumber($number)
    {
        ...
        // Handle the rightmost 3 digits (thousands)
        if ($length > 3) {
            $formatted = ',' . substr($remaining, -3);
            $remaining = substr($remaining, 0, -3);
        }

        // Handle remaining digits in pairs from right to left
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        $payment_types = ['cash' => __('lang_v1.cash'), 'card' => __('lang_v1.card'), 'cheque' => __('lang_v1.cheque'), 'bank_transfer' => __('lang_v1.bank_transfer'), 'credit_note' => __('lang_v1.credit_note'), 'other' => __('lang_v1.other')];
    ...
    public function generateReferenceNumber($type, $ref_count, $business_id = null, $default_prefix = null)
    {
        ...
        if (! in_array($type, ['contacts', 'business_location', 'username'])) {
            ...
            $ref_year = \Carbon::now()->month <= 3 ? \Carbon::now()->year - 1 : \Carbon::now()->year;
    ...
    {
        $business_location = BusinessLocation::where('business_id', auth()->user()['business_id'])->get()[0];
        ...
        if(!empty(auth()->user()["custom_field_1"])){
            $request_data[$sms_settings['param_2']] = auth()->user()["custom_field_1"];
        } else {
...
// dd($waiters->get());
     ...
     * @return array
     */
    public function replaceTags($business_id, $data, $transaction, $contact = null, $paid_at_once = "", $payment_ref_no ="")
    {
        $org_data = $data;
        ...
        foreach ($data as $key => $value) {
            if(strpos($value, '{transaction_date}') !== false) {
                $new_date = \Carbon\Carbon::parse(explode(" ", $transaction["transaction_date"])[0])->format("d-F-Y");
                $new_time = \Carbon\Carbon::parse(explode(" ", $transaction["transaction_date"])[1])->format("g:i A");

                 $data[$key] = str_replace("{transaction_date}", $new_date. " " . $new_time, $data[$key]);
            }
            if(strpos($value, '{purchase_ref_number}') !== false){
                $data[$key] = str_replace("{purchase_ref_number}", $transaction["ref_no"], $data[$key]);
            }
            
            if(strpos($value, '{product_name}') !== false){
                $product = PurchaseLine::where("transaction_id", $transaction->id)->first()->product;

                $data[$key] = str_replace("{product_name}", $product["name"], $data[$key]);
            }

            if(strpos($value, '{quantity}') !== false) {
                  $purchase = PurchaseLine::where("transaction_id", $transaction->id)->first();

                $data[$key] = str_replace("{quantity}", $purchase["quantity"], $data[$key]);
            }
              //Replace Reward Point earned
            if (strpos($value, '{earned_reward_point}') !== false) {
                $data[$key] = str_replace('{earned_reward_point}', $transaction->rp_earned === null ?  0 : $transaction->rp_earned, $data[$key]);
            }
             //Total Replace Reward Point
            if (strpos($value, '{total_reward_point}') !== false) {
        ...
        $details = ['points' => $customer_reward_points, 'amount' => $equivalent_amount];
                ...
                $data[$key] = str_replace('{total_reward_point}', $point === null ?  0 : $point, $data[$key]);
            }
            ...
            if($transaction['payment_type'] === "credit"){
                ...
                $payment_ref_number[0] = $transaction['payment_ref_no'];
            } else {
                if (! empty($transaction)) {
                    // dd($transaction->payment_lines);
                    
                    if(count($transaction->payment_lines) > 0 ) {
                    $is_same_date_and_time_transaction = $paid_at_once === "" ? false: true;
                    
                    if($is_same_date_and_time_transaction){
                        $total_paid = $paid_at_once;
                        if($payment_ref_no == ""){
                    ...
                    $payment_ref_number[0] = $payment_ref_no;
                }
                    }else {
                    $total_paid = $transaction->payment_lines[count($transaction->payment_lines) - 1]->amount;
                    }
                    }
                }
            }
            ...
            if (strpos($value, '{cumulative_due_amount}') !== false) {
                if($transaction["payment_type"] === "credit"){
                    $due = $this->getContactDue($transaction->contact->id);
            ...
            if (! empty($transaction->location)) {
                ...
                if (strpos($value, '{location_phone}') !== false) {
                    $location_phone = $transaction->location->moile;
