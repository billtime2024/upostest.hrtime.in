...
class TransactionUtil extends Util
{
    ...
    public function deleteSellLines($transaction_line_ids, $location_id, $adjust_qty = true)
    {
        if (! empty($transaction_line_ids)) {
            ...
            // Unmark IMEI/Serial numbers as available before deleting sell lines
            $deleted_sell_line_ids = $sell_lines->pluck('id');
            \App\ImeiNumber::whereIn('sell_line_id', $deleted_sell_line_ids)
                ->get()
                ->each(function ($imei_number) {
                    $imei_number->markAsUnsold();
                });
            ...
            if (! empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        ...
                        'credit_note_number' => isset($payment['credit_note_number']) ? $payment['credit_note_number'] : null,
                        'credit_note_type' => isset($payment['credit_note_type']) ? $payment['credit_note_type'] : null,
        ...
        if (! empty($payments_formatted)) {
            ...
            foreach ($account_transactions as $account_transaction) {
                ...
                if (! empty($payment)) {
                    ...
                    $notificationUtil->autoSendNotification($business_id  === null ? $transaction->business_id : $business_id, 'payment_paid', $transaction, $transaction->contact);
            }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_landmark == 1) {
            // $temp[] = $location_details->landmark . "</br>";
            $output['business_address'] = $location_details->landmark ;
        }
        if ($il->show_city == 1 && ! empty($location_details->city)) {
            $temp[] = $location_details->city;
            $output['business_city'] = $location_details->city;
        }
        if ($il->show_state == 1 && ! empty($location_details->state)) {
            $temp[] = $location_details->state;
            $output['business_state'] = $location_details->state;
        }
        if ($il->show_zip_code == 1 && ! empty($location_details->zip_code)) {
            $temp[] = $location_details->zip_code;
            $output['business_zip_code'] = $location_details->zip_code;
        }
        if ($il->show_country == 1 && ! empty($location_details->country)) {
            $temp[] = $location_details->country;
            $output['business_country'] = $location_details->country;
        }
        if (! empty($temp)) {
            $output['address'] = $location_details->landmark . ", </br>";
        ...
        if ($il->show_mobile_number == 1 && ! empty($location_details->mobile)) {
            ...
            $output['business_contact'] = $location_details->mobile;
        ...
        if ($il->show_alternate_number == 1 && ! empty($location_details->alternate_number)) {
            ...
            $output['business_alternate_contact'] = $location_details->alternate_number;
        }
        if ($il->show_email == 1 && ! empty($location_details->email)) {
            ...
            $output['business_email'] = $location_details->email;
        }
        ...
        if ($il->show_customer == 1) {
            ...
            $output['customer_name'] = ! empty($customer->supplier_business_name) ? $customer->supplier_business_name : $customer->name;
            ...
            $output['customer_address'] = $customer->address_line_1 . ", " . $customer->address_line_2 . ", " . $customer->city;
            ...
            if (! empty($customer->country)) {
                ...
                $output['customer_country']  = $customer->country;
            }
            if (! empty($customer->zip_code)) {
                $customer_address[] = '<br>'.$customer->zip_code;
                $output['customer_zip_code']  = $customer->zip_code;
            ...
            if (! empty($customer->landline)) {
                ...
                $output['customer_landline']  = $customer->landline;
            }
                        //  $output['customer_new_address'] = $output["customer_address"] . "</ br>" . $customer->city. ", ". 
                        // ", ". $output["customer_country"]. ", ".  $output["customer_zip_code"]. "</br>" . $output["customer_mobile"] . ", ". $output["customer_l"];
        ...
        if($output['shipping_address'] == null) {
            $output['shipping_address'] = $customer->shipping_address;
        }
            ...
            if ($il->show_payments == 1) {
                ...
                if (! empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        } elseif ($value['method'] == 'credit_note') {
                            $output['payments'][] =
                                ['method' => $method.(! empty($value['credit_note_number']) ? (', Credit Note Number:'.$value['credit_note_number']) : ''),
                                    'amount' => $this->num_f($value['amount'], $show_currency, $business_details),
                                    'date' => $this->format_date($value['paid_on'], false, $business_details),
                                ];
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            $product_details = Product::where('id', $line['product_id'])->get()[0];
          //  $line["product_custom_fields"] = $product["product_custom_field4"];
            ...
            $line_array = [
                ...
                'name' => $product->name,
                'product_custom_fields' => $product_details["product_custom_field4"],
            ...
            if (! empty($product->product_custom_field1) && in_array('product_custom_field1', $product_custom_fields_settings)) {
                ...
                $line_array['mrp'] = $product->product_custom_field1;
                $line_array['mrp_uf'] = $product->product_custom_field1;
            ...
            if (! empty($product->product_custom_field2) && in_array('product_custom_field2', $product_custom_fields_settings)) {
                ...
                $line_array['hsn'] = $product->product_custom_field2;
            }
            ...
            // Attach IMEI/Serial numbers (sold) to line array if present
            try {
                $imei_rows = $line->imei_numbers()->where('is_sold', 1)->get();
                if (! $imei_rows->isEmpty()) {
                    $imeis = $imei_rows->filter(function ($r) {
                        return $r->type === \App\ImeiNumber::TYPE_IMEI;
                    })->pluck('identifier')->filter()->values()->toArray();

                    $serials = $imei_rows->filter(function ($r) {
                        return $r->type === \App\ImeiNumber::TYPE_SERIAL;
                    })->pluck('identifier')->filter()->values()->toArray();

                    if (! empty($imeis)) {
                        $line_array['item_type'] = 'imei';
                        $line_array['sold_imei'] = $imeis;
                    } elseif (! empty($serials)) {
                        $line_array['item_type'] = 'serial';
                        $line_array['sold_serial'] = $serials;
                    }
                }
            } catch (\Exception $e) {
                // ignore failures while loading imei numbers
            }
...
// dd($permitted_locations);
            ...
            foreach ($due_transactions as $transaction) {
                ...
                if ($total_amount > 0) {
                    ...
                    $array = [
                        ...
                        'credit_note_number' => $parent_payment->credit_note_number,
                        'credit_note_type' => $parent_payment->credit_note_type,
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [], $permitted_locations = null)
    {
        ...
        $filters['sub_category_id'] = $filters['sub_category_id'] ?? [];
        $filters['brand_id'] = $filters['brand_id'] ?? [];
        $filters['unit_id'] = $filters['unit_id'] ?? [];
        if((gettype($filters['location_id']) === "array" &&
            gettype($filters["category_id"] === "array") &&
            gettype($filters["sub_category_id"] === "array") &&
            gettype($filters["brand_id"] === "array") &&
            gettype($filters["unit_id"] === "array")) && (
                count($filters['location_id']) === 1 &&
                count($filters['category_id']) === 1 &&
                count($filters['sub_category_id']) === 1 &&
                count($filters['brand_id']) === 1 &&
                count($filters['unit_id']) === 1
            )){
                $filters['location_id'] = $filters['location_id'][0];
                $filters['category_id'] = $filters['category_id'][0];
                $filters['sub_category_id'] = $filters['sub_category_id'][0];
                $filters['brand_id'] = $filters['brand_id'][0];
                $filters['unit_id'] = $filters['unit_id'][0];
            }
        if((
            gettype($filters["category_id"])  === "string" &&
            gettype($filters["sub_category_id"])  === "string" &&
            gettype($filters["brand_id"])  === "string" &&
            gettype($filters["unit_id"])  === "string" &&
            gettype($filters["location_id"])  === "string" ) || (
            gettype($filters['location_id']) === "NULL" &&
            gettype($filters["category_id"])  === "NULL" &&
            gettype($filters["sub_category_id"])  === "NULL" &&
            gettype($filters["brand_id"])  === "NULL" &&
            gettype($filters["unit_id"])  === "NULL"
       ) ){
            ...
            if (! empty($filters['location_id']) && $filters['location_id'] !== null) {
                $query->where('purchase.location_id', $filters['location_id']);
            }
            ...
            $query->leftjoin('variations as v', 'v.id', '=', 'purchase_lines.variation_id')
              ...
              $query->where(function ($query) use ($filters){
                    if (is_array($filters['category_id']) && count($filters['category_id']) > 0) {
                        foreach ($filters["category_id"] as $key => $cat_id) {
                            if($cat_id !== null){
                                if($key === 1){
                                    $query->where('p.category_id', $cat_id);
                                }else{
                                    $query->orWhere('p.category_id', $cat_id);
                                }
                            }
                        }
                    }
                ...
                $query->where(function ($query) use ($filters){
                    if (count($filters['sub_category_id']) > 0) {
                        foreach ($filters["sub_category_id"] as $key => $sub_cat_id) {
                            if($sub_cat_id !== null){
                                if($key === 1){
                                    $query->where('p.sub_category_id', $sub_cat_id);
                                }else{
                                    $query->orWhere('p.sub_category_id', $sub_cat_id);
                                }
                            }
                        }
                    }
              ...
              $query->where(function ($query) use ($filters){
                    if (count($filters['brand_id']) > 0) {
                        foreach ($filters["brand_id"] as $key => $brand_id) {
                            if($brand_id !== null){
                                if($key === 1){
              ...
              $query->where(function ($query) use ($filters){
                    if ( count($filters['unit_id']) > 0) {
                        foreach ($filters["unit_id"] as $key => $unit_id) {
                            if($unit_id !== null){
                                if($key === 1){
                                    $query->where('p.unit_id', $unit_id);
                                }else {
                                    $query->orWhere('p.unit_id', $unit_id);
                                }
                            }
                        }
                    }
            ...
            if(!empty($permitted_locations)) {
                if ($permitted_locations != 'all') {
                 ...
                 $query->where(function ($query) use ($filters){
                    if (count($filters['location_id']) > 0) {
                        foreach ($filters['location_id'] as $key => $loc_id) {
                            if($loc_id !== null){
                                if($key === 1){
                                    $query->where('purchase.location_id', $loc_id);
                                }else{
                                    $query->orWhere('purchase.location_id', $loc_id);
                                }
                            }
                        }
                    }
     ...
     * @return int
     */
    public function calculateRewardPoints($business_id, $transaction_id)
    {
        ...
        $sell_lines = TransactionSellLine::where('transaction_id', $transaction_id)
        ...
        foreach ($sell_lines as $line) {
            $product = $line->product;
            if ($product->enable_reward_points && $product->reward_points_per_amount > 0) {
                $line_total = $line->unit_price_inc_tax * $line->quantity;
                $points = floor($line_total / $product->reward_points_per_amount);
                $total_points += $points;
                $eligible_total += $line_total;
            }
        }

        if ($eligible_total < $business->min_order_total_for_rp) {
            $total_points = 0;
        }

        if (! empty($business->max_rp_per_order) && $total_points > $business->max_rp_per_order) {
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        if ($line_details) {
            $with = ['location', 'sell_lines', 'sell_lines.sub_unit', 'sell_lines.product',
                ...
                'purchase_lines.variations.product_variation', 'purchase_lines.line_tax', 'purchase_lines.note',
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null, $permitted_locations = null)
    {
        ...
        // Include ledger account transactions (income/expense) in profit & loss.
        // This will ensure ledger-only incomes (e.g., interest income) and expenses are considered.
        try {
            $ledger_income = AccountTransaction::join('accounts', 'account_transactions.account_id', '=', 'accounts.id')
                ...
                ->when(!empty($start_date) && !empty($end_date), function ($q) use ($start_date, $end_date) {
                    return $q->whereBetween(DB::raw('date(account_transactions.operation_date)'), [$start_date, $end_date]);
                })
                ->select(DB::raw("SUM(CASE WHEN account_transactions.type='credit' THEN account_transactions.amount ELSE -account_transactions.amount END) as net"))
                ->value('net') ?? 0;

            $ledger_expense = AccountTransaction::join('accounts', 'account_transactions.account_id', '=', 'accounts.id')
                ->join('account_types', 'accounts.account_type_id', '=', 'account_types.id')
                ->where('account_types.name', 'expense')
                ->where('accounts.business_id', $business_id)
                ->when(!empty($start_date) && !empty($end_date), function ($q) use ($start_date, $end_date) {
                    return $q->whereBetween(DB::raw('date(account_transactions.operation_date)'), [$start_date, $end_date]);
                })
                ->select(DB::raw("SUM(CASE WHEN account_transactions.type='debit' THEN account_transactions.amount ELSE -account_transactions.amount END) as net"))
                ->value('net') ?? 0;
        } catch (\Exception $e) {
            \Log::error('Failed to aggregate ledger income/expense: ' . $e->getMessage(), ['business_id' => $business_id]);
            $ledger_income = 0;
            $ledger_expense = 0;
        }
    
        // Expose ledger sums for downstream UI or modules
        $data['ledger_income'] = $ledger_income;
        $data['ledger_expense'] = $ledger_expense;
    
        // Combine ledger-based income/expense into net profit calculation so that
        // ledger-only entries (e.g., interest income posted to AccountTransaction) are included.
        $data['net_profit'] = $module_total + $gross_profit
                                + ($data['total_sell_round_off'] + $data['total_recovered'] + $data['total_sell_shipping_charge'] + $data['total_purchase_discount'] + $data['total_sell_additional_expense'] + $data['total_sell_return_discount'] + $data['ledger_income']
                                ) - ($data['total_reward_amount'] + $data['total_expense'] + $data['total_adjustment'] + $data['total_transfer_shipping_charges'] + $data['total_purchase_shipping_charge'] + $data['total_purchase_additional_expense'] + $data['total_sell_discount'] + $data['ledger_expense']
                                );
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        if($request->input('transaction_id') != null){
            $inputs['transaction_id'] = $request->transaction_id;
        }
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        if ($business->enable_rp == 1 && ! empty($sell->rp_earned)) {
            ...
            if (! $is_reward_expired) {
                ...
                $new_reward_point = $this->calculateRewardPoints($business_id, $sell->id);
