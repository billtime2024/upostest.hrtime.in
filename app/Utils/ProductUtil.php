...
class ProductUtil extends Util
{
    ...
    public function createOrUpdatePurchaseLines2($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        try {
            ...
            foreach ($input_data as $data) {
                ...
                $new_quantity = $this->num_uf($data['quantity']) * $multiplier;
                $lot_numbers = explode(',', $data['lot_number']);

                if (count($lot_numbers) !== count(array_unique($lot_numbers))) {
                    throw new \Exception("The lot numbers must be unique for product: {$data['product_id']}");
                }
                
                if (count($lot_numbers) !== (int)$new_quantity) {
                    throw new \Exception("The number of lot numbers must match the quantity for product: {$data['product_id']}");
                }

                foreach ($lot_numbers as $lot_number) {
                    $new_quantity_f = $this->num_f(1); // Each entry has a quantity of 1
                    $old_qty = 0;

                    // Check if the purchase line already exists
                    if (isset($data['purchase_line_id'])) {
                        $purchase_line = PurchaseLine::findOrFail($data['purchase_line_id']);
                        $updated_purchase_line_ids[] = $purchase_line->id;
                        $old_qty = $purchase_line->quantity;
                    ...
                    // Set properties for each lot number entry
                    $purchase_line->quantity = 1; // Quantity is always 1
                    $purchase_line->pp_without_discount = ($this->num_uf($data['pp_without_discount'], $currency_details) * $exchange_rate) / $multiplier;
                    $purchase_line->discount_percent = $this->num_uf($data['discount_percent'], $currency_details);
                    $purchase_line->purchase_price = ($this->num_uf($data['purchase_price'], $currency_details) * $exchange_rate) / $multiplier;
                    $purchase_line->purchase_price_inc_tax = ($this->num_uf($data['purchase_price_inc_tax'], $currency_details) * $exchange_rate) / $multiplier;
                    $purchase_line->item_tax = ($this->num_uf($data['item_tax'], $currency_details) * $exchange_rate) / $multiplier;
                    $purchase_line->tax_id = $data['purchase_line_tax_id'];
                    $purchase_line->lot_number = trim($lot_number);
                    $purchase_line->mfg_date = !empty($data['mfg_date']) ? $this->uf_date($data['mfg_date']) : null;
                    $purchase_line->exp_date = !empty($data['exp_date']) ? $this->uf_date($data['exp_date']) : null;
                    $purchase_line->sub_unit_id = !empty($data['sub_unit_id']) ? $data['sub_unit_id'] : null;
                    $purchase_line->purchase_order_line_id = !empty($data['purchase_order_line_id']) ? $data['purchase_order_line_id'] : null;
            ...
            // Handle deleted purchase lines and update database as in the original function
            $delete_purchase_line_ids = [];
            $delete_purchase_lines = null;
            if (!empty($updated_purchase_line_ids)) {
                $delete_purchase_lines = PurchaseLine::where('transaction_id', $transaction->id)
            ...
            \Log::error('Error in createOrUpdatePurchaseLines2: ' . $e->getMessage());
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        if (in_array('lot', $search_fields) || in_array('imei', $search_fields)) {
            $query->leftjoin('purchase_lines as pl', 'variations.id', '=', 'pl.variation_id');
        }
        if (in_array('imei', $search_fields)) {
            $query->leftjoin('imei_numbers', 'pl.id', '=', 'imei_numbers.purchase_item_id');
            // Added for IMEI/Serial search mirroring lot_number
        }
        ...
        if (! empty($search_term)) {
            ...
            if ($search_type == 'like') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    // if (in_array('variation', $search_fields)) {
                    //     $query->orWhereRaw(
                    //         "CONCAT(product_variations.name, '-', variations.name) LIKE ?",
                    //         ['%' . $search_term . '%']
                    //     );
                    // }


                         $query->orWhere('variations.name', 'like', '%'.$search_term.'%');
                    ...
                    if (in_array('imei', $search_fields)) {
                        $query->orWhere('imei_numbers.identifier', 'like', '%'.$search_term.'%');
                        // Added for IMEI/Serial search mirroring lot_number
                    }
            ...
            if ($search_type == 'exact') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    if (in_array('imei', $search_fields)) {
                        $query->orWhere('imei_numbers.identifier', $search_term);
                        // Added for IMEI/Serial search mirroring lot_number
                    }
                });
            }
        ...
        if (! empty($location_id)) {
            $query->ForLocation($location_id);
                ...
                if(request()->user()->can ("view_purchase_price")){
        $query->select(
                'products.id as product_id',
                'products.name',
                'products.type',
                ...
                'variations.sub_sku',
                DB::raw('MAX(U.short_name) as unit')
            );
                } else {
        $query->select(
                'products.id as product_id',
        ...
        // Added for IMEI/Serial search mirroring lot_number
        if (in_array('lot', $search_fields) || in_array('imei', $search_fields)) {
            $query->addSelect(DB::raw('MAX(pl.id) as purchase_line_id'));
        }
        if (in_array('lot', $search_fields) || in_array('imei', $search_fields)) {
            if (in_array('imei', $search_fields)) {
                $query->addSelect(DB::raw('MAX(COALESCE(pl.lot_number, imei_numbers.identifier)) as lot_number'));
            } else {
                $query->addSelect(DB::raw('MAX(pl.lot_number) as lot_number'));
            }
        }
        if ((in_array('lot', $search_fields) || in_array('imei', $search_fields)) && request()->user()->can("view_purchase_price")) {
            $query->addSelect(DB::raw('MAX(pl.purchase_price) as purchase_price'));
        }
        // Fixed persistent 500 error for search_fields incl. lot+imei (e.g. /products/list?search_fields[]=name&sku&lot&imei&term=t30&location_id=39)
        // Lot JOIN snippet: leftjoin('purchase_lines as pl', 'variations.id', '=', 'pl.variation_id'); leftjoin('imei_numbers', 'pl.id', '=', 'imei_numbers.purchase_line_id');
        // GROUP BY before: conditional ['variations.id', 'pl.id' [, 'imei_numbers.id']]
        // Cause: LEFT JOIN pl & imei_numbers causes row multiplication (multiple lots/IMEIs per variation); selected non-aggregated columns (pl.purchase_price, lot_number, qty_available etc.) violate strict SQL GROUP BY rules.
        // Fix: MAX() aggregates on joined SELECT fields + COALESCE(pl.lot_number, imei.identifier); retain extended GROUP BY covering PKs (pl.id, imei_numbers.id) for lot/imei.
        // Confirmed valid: lot alone OK (group variations.id, pl.id); imei alone OK (+imei.id); both OK (full group, no cartesian dupes).
        if (in_array('lot', $search_fields) || in_array('imei', $search_fields)) {
            // If searching for exact lot/imei, keep lot/imei level rows so user can pick the exact item.
            if ($search_type == 'exact') {
                $group = ['variations.id', 'pl.id'];
                if (in_array('imei', $search_fields)) {
                    $group[] = 'imei_numbers.id';
                }
                $query->groupBy($group);
            } else {
                // For non-exact searches collapse to variation level to avoid multiple rows per IMEI/lot.
        ...
        $results = $query->orderBy('VLD.qty_available', 'desc')->get();

        // Deduplicate identical display rows caused by JOINs (purchase_lines / imei_numbers)
        // By default collapse multiple rows per variation (caused by multiple lots/IMEIs) into a single variation entry.
        // If the user is performing an exact search for lot/imei (search_type == 'exact'), preserve lot/imei-level rows.
        $keep_lot_level = false;
        if ((in_array('lot', $search_fields) || in_array('imei', $search_fields)) && $search_type == 'exact') {
            $keep_lot_level = true;
        }

        if ($keep_lot_level) {
            // Keep lot/imei level rows distinct
            $results = $results->unique(function ($item) {
                return $item->product_id . '|' . $item->variation_id . '|' . ($item->lot_number ?? '');
            })->values();
        } else {
            // Collapse to variation level (one row per variation)
            $results = $results->unique(function ($item) {
                return $item->product_id . '|' . $item->variation_id;
            })->values();
        }

        // Set is_imei_match flag (useful when searching IMEI)
        foreach ($results as $result) {
            $result->is_imei_match = false;
            if (in_array('imei', $search_fields) && !empty($search_term)) {
                $imei_identifiers = $result->variations->pluck('imeiNumbers')->flatten()->pluck('identifier');
                $result->is_imei_match = $imei_identifiers->contains($search_term);
            }
        }
    ...
    public function getProductStockDetailsForMultipleSelectionFilter($business_id, $filters, $for)
    {
        // dd($filters);
           if(!isset($filters["location_id"])){
                    if(gettype($filters['category_id'] === "string")){
                        $filters['location_id'] = null;
                    }else {
                        $filters["location_id"] = array(0 => null);
                    }
                }
                ...
                if(!isset($filters["sub_category_id"])){
                     if(gettype($filters['category_id'] === "string")){
                        ...
                        $filters["sub_category_id"] = array(0 => null);
                    }
                }
        if(gettype($filters["location_id"]) === "string" ||
            ...
            return $this->getProductStockDetails($business_id, $filters, $for);
        } else{
                if(!isset($filters["location_id"])){
                    $filters["location_id"] = array(0 => null);
                }
                if(!isset($filters["category_id"])){
                    $filters["category_id"] = array(0 => null);
                }
                if(!isset($filters["brand_id"])){
                    $filters["brand_id"] = array(0 => null);
                }
                if(!isset($filters["unit_id"])){
                    $filters["unit_id"] = array(0 => null);
                }
                if(!isset($filters["sub_category_id"])){
                    $filters["sub_category_id"] = array(0 => null);
                }
            if(count($filters["location_id"]) === 1 &&
                count($filters["category_id"]) === 1 &&
                count($filters["sub_category_id"]) === 1 &&
                count($filters["brand_id"]) === 1 &&
                count($filters["unit_id"]) === 1
            ){
                $new_filters["location_id"] = $filters["location_id"][0] === null ? null : $filters["location_id"];
                $new_filters["category_id"] = $filters["category_id"][0] === null ? null  : $filters["category_id"];
                $new_filters["sub_category_id"] = $filters["sub_category_id"][0] === null ? null : $filters["sub_category_id"];
                $new_filters["brand_id"] = $filters["brand_id"][0] === null ? null : $filters["brand_id"];
                $new_filters["unit_id"] = $filters["unit_id"][0] === null ? null : $filters["unit_id"];
                if(isset($filters['stock_filter'])){
                    $new_filters["stock_filter"] = $filters["stock_filter"];
                }
                return $this->getProductStockDetails($business_id, $new_filters, $for);
            }else {
                if(!isset($filters["location_id"])){
                    $filters["location_id"] = array(0 => null);
                }
                ...
                if( gettype($filters["location_id"]) === "array" &&
                    ...
                    if ($permitted_locations != 'all') {
                     ...
                     $query->where(function ($query) use ($filters, $location_filter){
                        ...
                        foreach ($filters['location_id'] as $key => $loc_id) {
                            if($loc_id !== null){
                                if($key === 1){
                                    ...
                                    //If filter by location then hide products not available in that location
                                    // $query->orWhere('pl.location_id', $location_id);
                                    //$query->join('product_locations as pl', 'pl.product_id', '=', 'p.id')
                                    //      ->where(function ($q) use ($location_id) {
                                    //          $q->where('pl.location_id', $location_id);
                                    //      });
                ...
                $query->where(function ($query) use ($filters){
                    if (count($filters['category_id']) > 0) {
              ...
              $query->where(function ($query) use ($filters){
                    ...
                    if($filters["stock_filter"] === "exclude_zero_stock"){
                        $products = $query->select(
                        ...
                        )->having('stock', '>', 0)->groupBy('variations.id', 'vld.location_id');
                    }else if($filters["stock_filter"] === "include_zero_stock") {
                        $products = $query->select(
                            // DB::raw("(SELECT SUM(quantity) FROM transaction_sell_lines LEFT JOIN transactions ON transaction_sell_lines.transaction_id=transactions.id WHERE transactions.status='final' $location_filter AND
                            //     transaction_sell_lines.product_id=products.id) as total_sold"),
                            ...
                            DB::raw("(SELECT SUM(PL.quantity_returned) FROM transactions
                                      JOIN purchase_lines AS PL ON transactions.id=PL.transaction_id
                            ...
                            DB::raw("(SELECT SUM( COALESCE(pl.quantity - ($pl_query_string), 0) * purchase_price_inc_tax) FROM transactions
                                      ...
                                      WHERE (transactions.status='received' OR transactions.type='purchase_return') AND transactions.location_id=vld.location_id
                                      AND (pl.variation_id=variations.id)) as stock_price"),
                            DB::raw('COALESCE(total_purchased, 0) - COALESCE(total_purchase_return, 0) - COALESCE(total_sold, 0) + COALESCE(total_sell_return, 0) as stock'),
                            'b.name as brand_name',
                            'variations.sub_sku as sku',
                            'p.name as product',
                            'p.type',
                    ...
                    }else if($filters["stock_filter"] === "less_than_1_stock") {
                        $products = $query->select(
                            // DB::raw("(SELECT SUM(quantity) FROM transaction_sell_lines LEFT JOIN transactions ON transaction_sell_lines.transaction_id=transactions.id WHERE transactions.status='final' $location_filter AND
                            //     transaction_sell_lines.product_id=products.id) as total_sold"),
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        if (! empty($filters['location_id'])) {
            ...
            }else if (gettype($filters["location_id"]) === "array"){
        ...
        if(isset($filters["stock_filter"]) && $filters["stock_filter"] === "exclude_zero_stock"){
            $products = $query->select(
                // DB::raw("(SELECT SUM(quantity) FROM transaction_sell_lines LEFT JOIN transactions ON transaction_sell_lines.transaction_id=transactions.id WHERE transactions.status='final' $location_filter AND
                //     transaction_sell_lines.product_id=products.id) as total_sold"),
        ...
        }else if(isset($filters["stock_filter"]) && $filters["stock_filter"] === "include_zero_stock") {
            $products = $query->select(
                // DB::raw("(SELECT SUM(quantity) FROM transaction_sell_lines LEFT JOIN transactions ON transaction_sell_lines.transaction_id=transactions.id WHERE transactions.status='final' $location_filter AND
                //     transaction_sell_lines.product_id=products.id) as total_sold"),
        ...
        }else if(isset($filters["stock_filter"]) && $filters["stock_filter"] === "less_than_1_stock") {
            $products = $query->select(
                // DB::raw("(SELECT SUM(quantity) FROM transaction_sell_lines LEFT JOIN transactions ON transaction_sell_lines.transaction_id=transactions.id WHERE transactions.status='final' $location_filter AND
                //     transaction_sell_lines.product_id=products.id) as total_sold"),
