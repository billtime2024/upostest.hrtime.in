$(document).ready(function() {
    ...
    if(window.location.href.split("/")[window.location.href.split("/").length - 2] == "creditnote"){
...
$(document).on('click', '.auto_generate_lot_row', async function () {
    function randomChar (length) {
        const alpha = "ABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890";
        let result = "";
        ...
        try{
            const res = await fetch("/get_business_id");
            const data = await res.json();

    const buisnessId = data.id;
    const now = new Date();
    // Format the date and time as yearmonthdayhourminsec
    const baseLotNumber = 
        buisnessId +
        now.getFullYear().toString().slice(-2) +
        randomChar(7);
    // Find the lot type dropdown
    const lotType = $('#lot_type').val();

    
    const lotNumberField = $(this)
        .closest('tr')
        .find('textarea[name^="purchases"][name$="[lot_number]"]');

    if (lotType === 'single') {
        
        lotNumberField.val(baseLotNumber);
    } else if (lotType === 'multiple') {
        
        const quantityInput = $(this).closest('tr').find('.purchase_quantity');
        const quantity = parseInt(quantityInput.val(), 10);

        if (!isNaN(quantity) && quantity > 0) {
            const lotNumbers = [];
            for (let i = 1; i <= quantity; i++) {
                lotNumbers.push(`${baseLotNumber}-${String(i).padStart(2, '0')}`);
            }
            lotNumberField.val(lotNumbers.join(', '));
        } else {
            // If quantity is invalid, show an error
            toastr.error('Please enter a valid quantity to generate lot numbers.');
        }
    }
        }catch(e){
            console.log(e);
    ...
    $(document).on('click', '#purchase_list_filter_reset', function() {
        $('#purchase_list_filter_location_id').val('').trigger('change');
        $('#purchase_list_filter_supplier_id').val('').trigger('change');
        $('#purchase_list_filter_payment_status').val('').trigger('change');
        $('#purchase_list_filter_status').val('').trigger('change');
        $('#purchase_list_filter_date_range').val('').trigger('change');
        purchase_table.ajax.reload();
    });
...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        $.ajax({
            method: 'POST',
            url: window.location.href.split("/")[window.location.href.split("/").length - 2] == "creditnote" ?  "/creditnote/purchases/get_purchase_entry_row" : '/purchases/get_purchase_entry_row',
            dataType: 'html',
            data: data,
            success: function(result) {
...
function update_grand_total() {
    ...
    $('#discount_calculated_amount').text(__currency_trans_from_en(Math.abs(discount), true, true));
    ...
    $('#tax_calculated_amount').text(__currency_trans_from_en( Math.abs(tax), true, true));
    if(window.location.href.split("/")[window.location.href.split("/").length - 2 ] == "creditnote"){
        $("#total_discount").text(__currency_trans_from_en( Math.abs(parseFloat(tax) + ( -1 * parseFloat(discount))), true, true));
    }
    //Calculate shipping
...
$(document).on('click', 'button#submit_purchase_form', function(e) {
    ...
    if(window.location.href.split("/")[window.location.href.split("/") - 2] == "creditnote"){
        if($("#credit_note_type").val() == "product"){
            if ($('table#purchase_entry_table tbody tr').length <= 0) {
                toastr.warning(LANG.no_products_added);
                $('input#search_product').select();
                return false;
            }
        } else if($("#credit_note_type").val() == "discount") {
            if($("#discount_type").val() == null || $("#discount_amount").val() == "" || $("#tax_id").val() == ""){
    ...
    // Function to check if lot_type is set to "multiple"
    function isMultipleLotType() {
        return $('#lot_type').val() === 'multiple';
    }

    // Custom validation for mismatched rows
    function validateMismatchedRows() {
        let isValid = true;

        if (!isMultipleLotType()) {
            return isValid; // Skip validation if not "multiple"
        }

        $('table#purchase_entry_table tbody tr').each(function () {
            const row = $(this);
            const quantityInput = row.find('.purchase_quantity');
            const lotNumberTextarea = row.find("[name*='[lot_number]']");

            if (quantityInput.length && lotNumberTextarea.length) {
                const quantity = parseFloat(quantityInput.val() || 0);
                const lotNumbers = lotNumberTextarea
                    .val()
                    .split(',')
                    .filter((num) => num.trim() !== '');

                if (lotNumbers.length !== quantity) {
                    isValid = false;
                    row.addClass('mismatch-highlight'); // Highlight mismatched row
                } else {
                    row.removeClass('mismatch-highlight'); // Remove highlight if valid
                }
            }
        });
    ...
    // Custom validation for duplicate lot numbers
    function validateUniqueLotNumbers() {
        let lotNumbers = [];
        let isValid = true;
        ...
        // Collect all lot numbers from each row
        $('table#purchase_entry_table tbody tr').each(function () {
            const row = $(this);
            const lotNumberTextarea = row.find("[name*='[lot_number]']");

            if (lotNumberTextarea.length) {
                const lotNumbersInRow = lotNumberTextarea
                    .val()
                    .split(',')
                    .map((num) => num.trim())
                    .filter((num) => num !== '');

                // Add to the global list of lot numbers
                lotNumbers = lotNumbers.concat(lotNumbersInRow);
            }
        });

        // Check for duplicates
        const lotNumberCounts = lotNumbers.reduce((acc, lotNumber) => {
            acc[lotNumber] = (acc[lotNumber] || 0) + 1;
            return acc;
        }, {});

        // Find duplicates and highlight the corresponding fields
        const duplicates = Object.keys(lotNumberCounts).filter(
            (lotNumber) => lotNumberCounts[lotNumber] > 1
        );

        // Highlight the affected lot number fields
        $('table#purchase_entry_table tbody tr').each(function () {
            const row = $(this);
            const lotNumberTextarea = row.find("[name*='[lot_number]']");
            ...
            if (lotNumberTextarea.length) {
                ...
                // Highlight if any of the lot numbers in the row are duplicates
                lotNumbersInRow.forEach(function (lotNumber) {
                    if (duplicates.includes(lotNumber)) {
                        lotNumberTextarea.addClass('duplicate-lot-highlight');
                    } else {
                        lotNumberTextarea.removeClass('duplicate-lot-highlight');
                    }
                });
            }
        });
    ...
    $(window.location.href.split("/")[window.location.href.split("/").length - 2] == "creditnote" ? 'form#add_credit_note_form' : 'form#add_purchase_form').validate({
        rules: {
            ref_no: {
                remote: {
    ...
    if ($(window.location.href.split("/")[window.location.href.split("/").length - 2] == "creditnote" ? 'form#add_credit_note_form' : 'form#add_purchase_form').valid()) {
        ...
        $("html, body").animate({ scrollTop: 0 }, "slow");
         document.querySelector(".loader-con").style.display = "block";
        $(window.location.href.split("/")[window.location.href.split("/").length - 2] == "creditnote" ? 'form#add_credit_note_form' : 'form#add_purchase_form').submit();
    }
    //  document.querySelector(".loader-con").style.display = "none";
});
...
// IMEI Manager Modal Handlers
$(document).on('click', '.add-imei-btn, .manage-imei-btn', function() {
    currentImeiRow = $(this).data('row');
    imeiList = [];
    let isEditMode = $('#purchase_id').length > 0;
    let purchaseLineId = $(this).data('purchase-line-id');

    if (isEditMode && purchaseLineId) {
        // Load existing IMEIs from server for edit mode
        $.ajax({
            url: '/purchases/get-imei-numbers',
            method: 'POST',
            data: {
            ...
            success: function(data) {
                imeiList = data.map(item => ({imei: item.imei_number.trim(), sold: item.sold}));
                updateImeiModal();
                $('#imei_manager_modal').modal('show');
            },
            error: function() {
                toastr.error('Failed to load IMEI data');
            }
        });
    } else {
        // Load existing IMEIs if any for create mode
        let existingImeis = $('input[name="imei_numbers[' + currentImeiRow + '][]"]');
        if (existingImeis.length > 0) {
            imeiList = existingImeis.map(function() { return ({imei: $(this).val().trim(), sold: false}); }).get();
        }
        updateImeiModal();
        $('#imei_manager_modal').modal('show');
    }
...
$(document).on('click', '#add_imei_btn', function() {
    let imei = $('#imei_input').val().trim();
    if (addImeiToList(imei)) {
        $('#imei_input').val('').focus();
    }
});

$(document).on('click', '#import_bulk_imei_btn', function() {
    let bulkText = $('#bulk_imei_input').val().trim();
    if (bulkImportImeis(bulkText)) {
        $('#bulk_imei_input').val('');
    }
});

$(document).on('click', '#imei_manager_modal .delete-imei-btn', function() {
    let imei = $(this).data('imei').toString().trim();
    removeImeiFromList(imei);
});

$(document).on('click', '#save_imei_btn', function() {
    saveImeiData();
    $('#imei_manager_modal').modal('hide');
});

$(document).on('hidden.bs.modal', '#imei_manager_modal', function() {
    currentImeiRow = null;
    imeiList = [];
});

function addImeiToList(imei) {
    if (!imei) {
        toastr.error('Please enter an IMEI number');
        return false;
    }

    if (!/^\d{15}$/.test(imei)) {
        toastr.error('IMEI must be exactly 15 digits');
        return false;
    }

    if (imeiList.some(item => item.imei === imei)) {
        toastr.error('This IMEI is already in the list');
        return false;
    }

    // Get variation_id and location_id for validation
    let row = $('button[data-row="' + currentImeiRow + '"]').closest('tr');
    let variation_id = row.find('input[name*="variation_id"]').val() || row.data('variation-id');
    let location_id = $('#location_id').val();

    // Validate IMEI duplication
    let isValid = true;
    $.ajax({
        url: '/purchases/validate-imei',
        method: 'POST',
        data: {
            imei: imei,
            variation_id: variation_id,
        ...
        async: false, // Synchronous to prevent adding invalid IMEI
        ...
        error: function() {
            toastr.error('Failed to validate IMEI');
            isValid = false;
        }
    });
    ...
    imeiList.push({imei: imei, sold: false});
    updateImeiModal();
    return true;
}

function bulkImportImeis(bulkText) {
    if (!bulkText) {
        toastr.error('Please enter IMEI numbers');
        return false;
    }

    let imeis = bulkText.split('\n').map(imei => imei.trim()).filter(imei => imei);

    // Check for duplicates within the bulk input
    let uniqueImeis = [...new Set(imeis)];
    if (uniqueImeis.length !== imeis.length) {
        toastr.error('Duplicate IMEI numbers found in bulk input');
        return false;
    }
    ...
    imeis.forEach(imei => {
        if (!/^\d{15}$/.test(imei)) {
        ...
        } else if (imeiList.some(item => item.imei === imei)) {
            ...
            $.ajax({
                ...
                async: false, // Synchronous to validate each IMEI
                success: function(response) {
                    if (response.valid) {
                        validImeis.push(imei);
                    } else {
                        duplicationErrors.push(response.message);
                    }
                },
                error: function() {
                    duplicationErrors.push('Failed to validate IMEI: ' + imei);
                }
            });
        }
    });
    ...
    if (invalidImeis.length > 0) {
        toastr.error('Invalid IMEI format (must be 15 digits): ' + invalidImeis.join(', '));
        return false;
    }

    if (duplicateImeis.length > 0) {
        toastr.error('Duplicate IMEIs in list: ' + duplicateImeis.join(', '));
        return false;
    }

    if (duplicationErrors.length > 0) {
        toastr.error('Duplication errors: ' + duplicationErrors.join('; '));
        return false;
    }

    imeiList = imeiList.concat(validImeis.map(imei => ({imei: imei, sold: false})));
    updateImeiModal();
    toastr.success('Imported ' + validImeis.length + ' IMEI(s)');
    return true;
}

function removeImeiFromList(imei) {
    imei = imei.trim();
    imeiList = imeiList.filter(item => item.imei.trim() !== imei);
    updateImeiModal();
}

function updateImeiModal() {
    $('#imei_count').text(imeiList.length);
    let tbody = $('#imei_list_body');
    tbody.empty();

    imeiList.forEach(item => {
        let rowClass = item.sold ? 'text-muted bg-light' : '';
        let removeBtnDisabled = item.sold ? 'disabled' : '';
        let statusBadge = item.sold ? '<span class="badge badge-warning">Sold</span>' : '<span class="badge badge-success">Available</span>';
        tbody.append(
            '<tr class="' + rowClass + '">' +
                '<td>' + item.imei + '</td>' +
                '<td>' + statusBadge + '</td>' +
                '<td>' +
                    '<button type="button" class="btn btn-sm btn-danger delete-imei-btn" data-imei="' + item.imei + '" ' + removeBtnDisabled + '>' +
...
function saveImeiData() {
    if (currentImeiRow === null) return;

    let isEditMode = $('#purchase_id').length > 0;

    // Remove existing IMEI inputs
    $('input[name="imei_numbers[' + currentImeiRow + '][]"]').remove();

    // Add new IMEI inputs
    imeiList.forEach(item => {
        let input = $('<input>')
            .attr('type', 'hidden')
            .attr('name', 'imei_numbers[' + currentImeiRow + '][]')
            .val(item.imei);
        $('#purchase_entry_table').append(input);
    });

    // Update quantity to match IMEI count (for both create and edit modes)
    let row = $('button[data-row="' + currentImeiRow + '"], button[data-purchase-line-id]').closest('tr');
    row.find('.purchase_quantity').val(imeiList.length);
    if (!isEditMode) {
...
function validateIdentifiers() {
    ...
    $('table#purchase_entry_table tbody tr').each(function(index) {
        let row = $(this);
        let itemType = row.data('item-type') || row.find('input[name="purchases[' + index + '][item_type]"]').val();
        let quantity = parseInt(row.find('.purchase_quantity').val()) || 0;
        if (itemType === 'imei' || itemType === 'serial') {
            let identifiers = row.find('input[name="' + (itemType === 'imei' ? 'imei' : 'serial') + '_numbers[' + index + '][]"]').map(function() { return $(this).val().trim(); }).get().filter(v => v);
            if (!isEditMode && identifiers.length !== quantity) {
                isValid = false;
                toastr.error('Identifier count does not match quantity for row ' + (index + 1));
            }
            if (identifiers.length > 0 && new Set(identifiers).size !== identifiers.length) {
                isValid = false;
                toastr.error('Duplicate identifiers for row ' + (index + 1));
            }
        }
    });
    return isValid;
...
// Serial Manager Modal Handlers
$(document).on('click', '.add-serial-btn, .manage-serial-btn', function() {
    currentSerialRow = $(this).data('row');
    serialList = [];
    let isEditMode = $('#purchase_id').length > 0;
    let purchaseLineId = $(this).data('purchase-line-id');

    if (isEditMode && purchaseLineId) {
        // Load existing serials from server for edit mode
        $.ajax({
            url: '/purchases/get-serial-numbers',
            method: 'POST',
            data: {
                purchase_line_id: purchaseLineId,
                _token: $('meta[name="csrf-token"]').attr('content')
            ...
            success: function(data) {
                serialList = data.map(item => ({serial: item.serial_number.trim(), sold: item.sold}));
                updateSerialModal();
                $('#serial_manager_modal').modal('show');
            },
            error: function() {
                toastr.error('Failed to load serial data');
            }
        });
    } else {
        // Load existing serials if any for create mode
        let existingSerials = $('input[name="serial_numbers[' + currentSerialRow + '][]"]');
        if (existingSerials.length > 0) {
            serialList = existingSerials.map(function() { return ({serial: $(this).val().trim(), sold: false}); }).get();
        }
        updateSerialModal();
        $('#serial_manager_modal').modal('show');
    }
...
$(document).on('click', '#add_serial_btn', function() {
    let serial = $('#serial_input').val().trim();
    if (addSerialToList(serial)) {
        $('#serial_input').val('').focus();
    }
});

$(document).on('click', '#import_bulk_serial_btn', function() {
    let bulkText = $('#bulk_serial_input').val().trim();
    if (bulkImportSerials(bulkText)) {
        $('#bulk_serial_input').val('');
    }
});

$(document).on('click', '#serial_manager_modal .delete-serial-btn', function() {
    let serial = $(this).data('serial').toString().trim();
    removeSerialFromList(serial);
});

$(document).on('click', '#save_serial_btn', function() {
    saveSerialData();
    $('#serial_manager_modal').modal('hide');
});

$(document).on('hidden.bs.modal', '#serial_manager_modal', function() {
...
function addSerialToList(serial) {
    if (!serial) {
        toastr.error('Please enter a serial number');
        return false;
    }

    if (serialList.some(item => item.serial === serial)) {
        toastr.error('This serial is already in the list');
        return false;
    }

    // Get variation_id and location_id for validation
    let row = $('button[data-row="' + currentSerialRow + '"]').closest('tr');
    let variation_id = row.find('input[name*="variation_id"]').val() || row.data('variation-id');
    let location_id = $('#location_id').val();

    // Validate serial duplication
    let isValid = true;
    $.ajax({
        url: '/purchases/validate-serial',
        method: 'POST',
        data: {
            serial: serial,
            variation_id: variation_id,
        ...
        async: false, // Synchronous to prevent adding invalid serial
        success: function(response) {
            if (!response.valid) {
                toastr.error(response.message);
                isValid = false;
        ...
        error: function() {
            toastr.error('Failed to validate serial');
            isValid = false;
        }
    });
    ...
    serialList.push({serial: serial, sold: false});
    updateSerialModal();
    return true;
}

function bulkImportSerials(bulkText) {
    if (!bulkText) {
        toastr.error('Please enter serial numbers');
        return false;
    }

    let serials = bulkText.split('\n').map(serial => serial.trim()).filter(serial => serial);

    // Check for duplicates within the bulk input
    let uniqueSerials = [...new Set(serials)];
    if (uniqueSerials.length !== serials.length) {
        toastr.error('Duplicate serial numbers found in bulk input');
        return false;
    }
    ...
    serials.forEach(serial => {
        if (serialList.some(item => item.serial === serial)) {
            ...
            $.ajax({
                ...
                async: false, // Synchronous to validate each serial
                success: function(response) {
                    if (response.valid) {
                ...
                error: function() {
                    duplicationErrors.push('Failed to validate serial: ' + serial);
                }
            });
        }
    });
    ...
    if (duplicateSerials.length > 0) {
        toastr.error('Duplicate serials in list: ' + duplicateSerials.join(', '));
        return false;
    }
    ...
    serialList = serialList.concat(validSerials.map(serial => ({serial: serial, sold: false})));
    updateSerialModal();
    toastr.success('Imported ' + validSerials.length + ' serial(s)');
    return true;
}

function removeSerialFromList(serial) {
    serial = serial.trim();
    serialList = serialList.filter(item => item.serial.trim() !== serial);
    updateSerialModal();
}

function updateSerialModal() {
    $('#serial_count').text(serialList.length);
    let tbody = $('#serial_list_body');
    tbody.empty();
    ...
    serialList.forEach(item => {
        ...
        tbody.append(
            '<tr class="' + rowClass + '">' +
                ...
                '<td>' + statusBadge + '</td>' +
                '<td>' +
                    '<button type="button" class="btn btn-sm btn-danger delete-serial-btn" data-serial="' + item.serial + '" ' + removeBtnDisabled + '>' +
                        '<i class="fa fa-trash"></i>' +
                    '</button>' +
                '</td>' +
            '</tr>'
...
function saveSerialData() {
    if (currentSerialRow === null) return;

    let isEditMode = $('#purchase_id').length > 0;

    // Remove existing serial inputs
    $('input[name="serial_numbers[' + currentSerialRow + '][]"]').remove();

    // Add new serial inputs
    serialList.forEach(item => {
        let input = $('<input>')
            .attr('type', 'hidden')
            .attr('name', 'serial_numbers[' + currentSerialRow + '][]')
            .val(item.serial);
        $('#purchase_entry_table').append(input);
    });

    // Update quantity to match serial count (for both create and edit modes)
    let row = $('button[data-row="' + currentSerialRow + '"], button[data-purchase-line-id]').closest('tr');
    row.find('.purchase_quantity').val(serialList.length);
    if (!isEditMode) {
        row.find('.purchase_quantity').trigger('change');
    }
}
...
function unsavedChangesExist() {
    const tbody = document.getElementById("purchase_entry_table").querySelector("tbody");
...
window.addEventListener('beforeunload', function(event) {
    if (unsavedChangesExist() && event.url !== document.activeElement.href) {
