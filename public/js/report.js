$(document).ready(function() {
   let totalPurchasePriceChart;
   let totalSeviceStaffChart;
    
    if(window.location.href.split("/")[window.location.href.split("/").length - 1] == "product-sell-report"){
 totalSeviceStaffChart = Highcharts.chart('container_service_staff_detailed', {
                            chart: {
                            ...
                            title: {
                                text: 'Service Staff Wise Total Sale'
                            },
                              exporting: {
                                    enabled: false
                                  },
                            tooltip: {
                                pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b> <span  style="color: #00359e; font-size: 1.25rem;">  (₹{point.y})</span>'
                            },
                            accessibility: {
                                point: {
                                    valueSuffix: '%'
 ...
 totalPurchasePriceChart = Highcharts.chart('container_purhcase_price', {
chart: {
    plotBackgroundColor: null,
    plotBorderWidth: null,
    plotShadow: false,
...
title: {
    text: 'Service Staff Wise Profit '
},
tooltip: {
    pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b> <span style="color: #00359e; font-size: 1.25rem;">  (₹{point.y})</span>'
},
...
series: [{
    ...
    var stock_report_cols = [
        ...
        stock_report_cols.push({ data: 'imei_serial', name: 'imei_serial', searchable: false });
        stock_report_cols.push({ data: 'lot_number', name: 'lot_number', searchable: false });
    ...
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                if($("#show_lot_number")){
                    d.show_lot_number = $("#show_lot_number").is(':checked') ? 1 : 0;
                }
    ...
    $('#stock_report_filter_form #stock_filter').change(function (e) {
    ...
    $('#stock_report_filter_form #show_lot_number').on('ifChanged', function (e) {
        stock_report_table.ajax.reload();
        stock_expiry_report_table.ajax.reload();
        get_stock_value();
    })
    ...
    if ($('table#product_sell_report_table').length == 1) {
        ...
        product_sell_report = $('table#product_sell_report_table').DataTable({
            ...
            ajax: {
                ...
                // if($('#psr_product_custom_field1').html().trim().length > 0) {
                //     $('#psr_product_custom_field1').hide();
                // }
                //  if($('#psr_product_custom_field2').html().trim().length > 0) {
                //     $('#psr_product_custom_field2').hide();
                // }
                 },
            },
            columns: [
                ...
                { data: 'service_staff', name: 'transaction_sell_lines.res_service_staff_id'},
            ...
            fnDrawCallback: function(oSettings) {
                ...
                const newServiceStaffData = [];
                const serviceNameData = [];
                const newServiceStaffNameData = [];
                
                data.forEach((serviceStaff, index) => {
                    const name = serviceStaff.service_staff;
                    const productName = serviceStaff.product_name;
                    const y = parseFloat(JSON.parse(serviceStaff.subtotal.split(" ")[3].split("=")[1].split(">")[0]));
                    const purchasePrice = parseFloat(JSON.parse(serviceStaff.purchase_price.split(" ")[4].split(">")[1].split("<")[0] === "" ? 0 :serviceStaff.purchase_price.split(" ")[4].split(">")[1].split("<")[0]));

                    if(serviceStaffData[name ] == undefined){
                        serviceStaffData[name] = Math.round(y);
                    }else {
                      serviceStaffData[name] += Math.round(y);
                    }
                    
                    if(serviceNameData[name] == undefined) {
                        serviceNameData[name] = Math.round(y - purchasePrice);
                    } else {
                        serviceNameData[name] += Math.round(y - purchasePrice);
                    }
                })
        
                for (const property in serviceStaffData) {
                    newServiceStaffData.push({name: property, y: serviceStaffData[property]});
                }
                
                for(const property in serviceNameData) {
                    if(serviceNameData[property] > 0 ){
                        newServiceStaffNameData.push({name: property, y: Math.abs(serviceNameData[property])});
                    }
                }
                totalPurchasePriceChart.series[0].setData(newServiceStaffNameData, true);
                totalSeviceStaffChart.series[0].setData(newServiceStaffData, true);
            },
        });
    }
    ...
    lot_report = $('table#lot_report').DataTable({
        ...
        columns: [
            ...
            { data: 'variation_name', name: 'v.name' },
            { data: 'lot_number', name: 'pl.lot_number' },
            ...
            { data: "purchase_price_inc_tax", name: "pl.purchase_price_inc_tax", searchable: false},
            { data: "created_at", name: "pl.created_at" },
            { data: "stock_age", name: "stock_age" },
        ],
        ...
        fnDrawCallback: function(oSettings) {
            ...
            // $('#footer_total_purchased').html('₹' + __sum_stock($('#lot_report'), 'total_purchased' ).replace("undefined", ""));
            $('#footer_total_purchased').html(__sum_stock($('#lot_report'), 'total_purchased' ).replace("undefined", ""));
    ...
    if ($('table#lot_report').length == 1) {
        $('#location_id, #category_id, #sub_category_id, #unit, #brand, #stock_filter').change(function() {
            lot_report.ajax.reload();
        });
    }
...
function get_stock_value() {
    ...
    $('#closing_stock_by_pp_round').html(loader);
    $('#closing_stock_by_sp_round').html(loader);
    $('#potential_profit_round').html(loader);
    $('#profit_margin_round').html(loader);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#closing_stock_by_pp_round').text(__currency_trans_from_en(data.closing_stock_by_pp_round.split(" ")[0])+ "    " + data.closing_stock_by_pp_round.split(" ")[1]);
        
            $('#closing_stock_by_sp_round').text(__currency_trans_from_en(data.closing_stock_by_sp_round.split(" ")[0]) +"    " + data.closing_stock_by_sp_round.split(" ")[1]);
            $('#potential_profit_round').text(__currency_trans_from_en(data.potential_profit_round.split(" ")[0]) + "    " +data.potential_profit_round.split(" ")[1]);
            $('#profit_margin_round').text(__currency_trans_from_en(data.profit_margin_round, false) + " %");
        },
    });
}
