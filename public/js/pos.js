...
$(document).ready(function() {
    ...
    if ($('#search_product').length) {
        ...
        $('#search_product')
            ...
            if (item.enable_stock == 1 && item.qty_available <= 0 && !is_overselling_allowed && !for_so && !is_draft) {
                ...
                if(selling_price){
                    string += '<b>[ Sell Price: ' + __currency_trans_from_en(selling_price, false, false, __currency_precision, true) + ']</b>';
                    string += '<b>[ LPP: ' + __currency_trans_from_en(item.purchase_price, false, false, __currency_precision, true) + ']</b>';
                }
                string += '</div>';
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        if(entered_qty < 1){
            toastr.error("Quantity can not be  less then 1");
            __write_number(tr.find('input.pos_quantity'), Math.abs(entered_qty), false);
            return;
        }

        var unit_price_inc_tax = __read_number(tr.find('input.pos_unit_price_inc_tax'));
        var line_total = Math.abs(entered_qty * unit_price_inc_tax);
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price', function() {
        ...
        var line_total = Math.abs(quantity * unit_price_inc_tax);

        __write_number(tr.find('input.pos_unit_price_inc_tax'), Math.abs(unit_price_inc_tax));
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price_inc_tax', function() {
        ...
        if (iraqi_selling_price_adjustment) {
            unit_price_inc_tax = Math.abs(round_to_iraqi_dinnar(unit_price_inc_tax));
        ...
        __write_number(tr.find('input.pos_unit_price'), Math.abs(unit_price));
        __write_number(tr.find('input.pos_line_total'), Math.abs(line_total), false);
    ...
    $('button#pos-finalize').click(function() {
        ...
        const customer_id_with_brackets = $("#select2-customer_id-container").html().split(" ")[$("#select2-customer_id-container").html().split(" ").length - 1]
        let new_customer_id = "";
        for(let i = 0; i < customer_id_with_brackets.length; i++){
            if(i !== 0 && i !== customer_id_with_brackets.length - 1){
                new_customer_id += customer_id_with_brackets[i];
            }
        }
        $.ajax({
            url: '/contacts/customers?q=' + new_customer_id,
            dataType: 'json',
            delay: 250,
            data: function(params) {
                return {
            ...
            success: function(data) {
                const advance_payment = parseFloat(data[0].balance);
                const total_payable = parseFloat($("#total_payable").html());
                if(advance_payment > total_payable){
                    toastr.success("Your Advance Balance is more than payable amount");
                }
                return {
                    results: data,
                };
        ...
        submitHandler: function(form) {
            ...
            if (cnf) {
                ...
                dataArray.forEach(singleData => {
                    if(singleData.name.includes("payment") && singleData.name.includes("method")) {
                        if(singleData.value === "none") {
                            isError = true;
                            toastr.error("Payment Method can not be none");
                            document.querySelector(".loader-con").style.display = "none";
                            document.getElementById("pos-save").disabled = false;
                        }
                    }
                })
                console.log("time when start pos: ", new Date().toString().split(" ")[4]);
                const test = true;
                if(!isError) {
                    $.ajax({
                        method: 'POST',
                        ...
                        success: function(result) {
                            ...
                            if (result.success == 1) {
                                ...
                                //Check if enabled or not
                                if (result.receipt.is_enabled) {
                                    console.log("time when start print: ", new Date().toString().split(" ")[4]);
                                    pos_print(result.receipt);
                                    console.log("time when end print: ", new Date().toString().split(" ")[4]);
                                }
                            } else {
                                toastr.error(result.msg);
                                
                            }
                            console.log("time when end pos: ", new Date().toString().split(" ")[4]);
                            enable_pos_form_actions();
                        },
                    });
                    } else {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (result.success) {
                    ...
                    // Auto-open IMEI/Serial modal if product type is IMEI or Serial
                    if (result.item_type && ['imei', 'serial'].includes(result.item_type)) {
                        var row_index = this_row.data('row_index');
                        var variation_id = this_row.find('.row_variation_id').val();
                        var item_type = result.item_type;
                        var locationId = $('input#location_id').val();
                        var modal = $('#imei-serial-modal');
                        modal.data('row-index', row_index);
                        modal.find('.modal-title').text('Select ' + (item_type == 'imei' ? 'IMEI' : 'Serial') + ' Numbers');
                            ...
                            success: function(response) {
                                ...
                                if (response.success && response.data.length > 0) {
                                    response.data.forEach(function(item) {
                                        listHtml += '<div class="checkbox"><label><input type="checkbox" class="imei-serial-checkbox" value="' + item.identifier + '" > ' + item.identifier + '</label></div>';
                                    });
                                } else {
                                    listHtml = '<p>No available ' + item_type + ' numbers.</p>';
                                }
...
function calculate_billing_details(price_total) {
    ...
    document.getElementById("final_total_input").dispatchEvent(new Event('change'));
    var curr_exchange_rate = 1;
...
$(document).on('change', '.payment_types_dropdown', function(e) {
    ...
    if (payment_type && payment_type != 'advance' && payment_type != "none") {
        var default_account = '';
...
$(document).on('submit', 'form#add_expense_modal_form', function(e) {
    e.preventDefault();
     if($("#add_expense_modal_form  #method_0").val() != "none"){
        ...
        toastr.error("Select Valid Payment Method", "Error");
        $("#add_expense_modal_form button[disabled='disabled'").removeAttr("disabled");
    }
});
...
$(document).on('click', '#send_for_sell_return', function(e) {
    ...
    invoice_no = invoice_no.split("/").join(":");
    if (invoice_no) {
        $.ajax({
            method: 'get',
...
// IMEI/Serial selection modal handlers (fixed)
$(document).on('show.bs.modal', '.modal[id^="select_imei_serial_modal_"]', function() {
    var modal = $(this);
    var row_index = modal.data('row-index');
    var variation_id = modal.data('variation-id');
    var item_type = modal.data('item-type');
    var sell_line_id = modal.data('sell-line-id') || null;

    // Fetch available IMEI/Serial (use location-aware endpoint and send location)
    $.ajax({
        url: '/sells/pos/get-available-imei-serial-numbers',
        method: 'GET',
        data: {
        ...
        success: function(response) {
            var data = response.data || response.available || [];
            var selected = response.selected || [];
            var selectedIdentifiers = selected.map(function(s) { return (s && s.identifier) ? s.identifier : s; });

            var list_html = '';
            data.forEach(function(item) {
                var identifier = item.identifier || item;
                var checked = selectedIdentifiers.indexOf(identifier) !== -1 ? 'checked' : '';
                list_html += '<div class="checkbox"><label><input type="checkbox" class="imei-serial-checkbox" value="' + identifier + '" ' + checked + '> ' + identifier + '</label></div>';
            });

            modal.find('#imei_serial_list_' + row_index).html(list_html);
            update_selected_quantity(modal, row_index);
        },
        error: function(xhr, status, err) {
            console.error('Failed to fetch IMEI/Serial numbers', xhr, status, err);
        }
    });
});

$(document).on('change', '.imei-serial-checkbox', function() {
    var modal = $(this).closest('.modal');
    var row_index = modal.data('row-index');
    update_selected_quantity(modal, row_index);
});

function update_selected_quantity(modal, row_index) {
    var selected = modal.find('.imei-serial-checkbox:checked').length;
    modal.find('#selected_quantity_' + row_index).val(selected);
}

$(document).on('click', 'button[id^="save_imei_serial_"]', function() {
    var button = $(this);
    var row_index = button.attr('id').split('_').pop();
    var modal = button.closest('.modal');
    var selected_count = modal.find('.imei-serial-checkbox:checked').length;

    // Update the quantity input
    $('input[name="products[' + row_index + '][quantity]"]').val(selected_count);
    // Trigger change
    $('input[name="products[' + row_index + '][quantity]"]').change();
