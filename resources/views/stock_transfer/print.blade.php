          ...
          <tr>
            ...
            <td>
              {{ $sell_lines->product->name }}
               ...
               @if(in_array($sell_lines->product->item_type, ['imei', 'serial']))
                <br><small>{{ $sell_lines->transferredImeiNumbers->pluck('identifier')->implode(', ') }}</small>
               @endif
...
<div class="row print_section">
  <div class="col-xs-12">
    <img class="center-block" src="data:image/png;base64,{{DNS1D::getBarcodePNG($sell_transfer->ref_no, 'C128', 2,30,array(39, 48, 54), true)}}">
            ...
            <div class="pull-right">For <strong>{{ $location_details['sell']->name }}</strong></div>
