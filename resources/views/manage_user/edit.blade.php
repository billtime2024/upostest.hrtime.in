...
<section class="content">
    ...
    <div class="row">
        ...
        @component('components.widget', ['title' => __('lang_v1.roles_and_permissions')])
            ...
            <div class="col-md-12">
              <h4>@lang('lang_v1.set_default_location') @show_tooltip(__('lang_v1.set_default_location_tooltip'))</h4>
              <div class="form-group">
                <div class="checkbox">
                  <label>
                    {!! Form::radio('default_location_id', null, empty($user->default_location_id), ['class' => 'input-icheck default-location-radio', 'disabled' => true]) !!} {{ __('lang_v1.none') }}
                  </label>
                </div>
                @foreach($locations as $location)
                <div class="checkbox">
                  <label>
                    {!! Form::radio('default_location_id', $location->id, $user->default_location_id == $location->id, ['class' => 'input-icheck default-location-radio', 'disabled' => $permitted_locations !== 'all' && !in_array($location->id, $permitted_locations)]) !!} {{ $location->name }} @if(!empty($location->location_id))({{ $location->location_id}}) @endif
                  </label>
                </div>
                @endforeach
              </div>
...
<script type="text/javascript">
  ...
  // Handle default location radio buttons
  function updateDefaultLocationRadios() {
    var selectedLocations = [];
    $('input[name="location_permissions[]"]:checked').each(function() {
      selectedLocations.push($(this).val().replace('location.', ''));
    });

    $('.default-location-radio').prop('disabled', true).prop('checked', false);

    // Enable "None" option
    $('input[name="default_location_id"][value=""]').prop('disabled', false);

    // Enable radios for selected locations
    selectedLocations.forEach(function(locationId) {
      $('input[name="default_location_id"][value="' + locationId + '"]').prop('disabled', false);
    });

    // Re-check the current default location if it's enabled
    var currentDefault = '{{ $user->default_location_id }}';
    if (currentDefault && $('input[name="default_location_id"][value="' + currentDefault + '"]').is(':not(:disabled)')) {
      $('input[name="default_location_id"][value="' + currentDefault + '"]').prop('checked', true);
    } else if (!currentDefault) {
      $('input[name="default_location_id"][value=""]').prop('checked', true);
    }
  }

  $('input[name="location_permissions[]"]').on('ifChecked ifUnchecked', function() {
    updateDefaultLocationRadios();
  });
