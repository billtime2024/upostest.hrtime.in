						...
						<table class="table table-condensed table-bordered text-center table-responsive table-striped add_opening_stock_table">
								...
								<tr class="bg-green">
									...
									@if(in_array($product->item_type, ['imei', 'serial']))
										<th>@lang($product->item_type == 'imei' ? 'lang_v1.imei_numbers' : 'lang_v1.serial_numbers')</th>
									@endif
...
@if(in_array($product->item_type, ['imei', 'serial']))
<td>
	{!! Form::textarea('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][' . ($product->item_type == 'imei' ? 'imei_numbers' : 'serial_numbers') . ']',
		!empty($var[$product->item_type . '_numbers']) ? implode("\n", $var[$product->item_type . '_numbers']) : '',
		['class' => 'form-control input-sm', 'rows' => 3, 'placeholder' => __('lang_v1.one_per_line')]); !!}
</td>
@endif
<td>
	...
	@if(in_array($product->item_type, ['imei', 'serial']))
	<td>
		<textarea rows="3" class="form-control input-sm" name="stocks[{{$key}}][{{$variation->id}}][__subkey__][{{ $product->item_type == 'imei' ? 'imei_numbers' : 'serial_numbers' }}]" placeholder="{{ __('lang_v1.one_per_line') }}"></textarea>
	</td>
	@endif
	<td>
	...
	@endforeach
								...
								<tfoot>
								<tr>
									<td colspan="{{ 6 + ($enable_expiry && $product->enable_stock ? 1 : 0) + ($enable_lot ? 1 : 0) + (in_array($product->item_type, ['imei', 'serial']) ? 1 : 0) }}"></td>
