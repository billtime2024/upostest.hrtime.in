...
<div class="col-md-6 col-sm-6 col-xs-6 text-right align-right @if(!empty($for_pdf)) width-50 f-left @endif">
	...
	<div style="border: 1px solid #000; padding: 10px;">
		...
		@php
		//$total = 0;
		//foreach($ledger_details as $key => $ledger_detail){
		//    $total += $ledger_detail['total_purchase'];
		// }
		// $ledger_details['all_total_purchase'] = $total;
		//dd($contact);
		@endphp
			...
			@php
		if( $contact->type == 'supplier' || $contact->type == 'both'){
	    $ledger_details["all_balance_due"] = ($ledger_details['total_purchase'] + $ledger_details["beginning_balance"]) - $ledger_details["total_paid"];
	  } else {
	     $ledger_details["all_balance_due"] = ($ledger_details["total_invoice"] + $ledger_details["beginning_balance"]) - $ledger_details["total_paid"];
	  }
	@endphp 
				<td><strong>@lang('lang_v1.balance_due')</strong></td>
				<td class="align-right">@format_currency($ledger_details['all_balance_due'] - $ledger_details['ledger_discount'])</td>
			</tr>
		</table>
	</div>
</div>
