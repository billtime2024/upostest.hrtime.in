      ...
      <div class="row">
        ...
        <div class="col-md-6">
          <div class="well">
            ...
            <strong>@lang('contact.total_purchase_due'): </strong><span class="display_currency" data-currency_symbol="true">{{ ($contact_details->opening_balance + $contact_details->total_purchase + $contact_details->total_return_paid)  - (
             $contact_details->total_paid + $contact_details->total_purchase_return + $contact_details->ledger_discount ) }}</span><br>
        ...
        @elseif(in_array($due_payment_type, ['sell']))
          ...
          <div class="col-md-6">
            <div class="well">
              ...
              <strong>@lang('contact.total_paid'): </strong><span class="display_currency" data-currency_symbol="true">{{ $contact_details->total_paid }}</span><br>
              <strong>@lang('contact.total_sale_due'): </strong><span class="display_currency" data-currency_symbol="true">{{ $contact_details->total_invoice - $contact_details->total_paid  - $contact_details->ledger_discount }}</span><br>
              @if(!empty($contact_details->opening_balance) || $contact_details->opening_balance != '0.00')
                  <strong>@lang('lang_v1.opening_balance'): </strong>
                  <span class="display_currency" data-currency_symbol="true">
                  {{ $contact_details->opening_balance }}</span><br>
        ...
        <div class="col-md-6">
          <div class="well">
            ...
            <strong>@lang('lang_v1.total_sell_return_due'): </strong><span class="display_currency" data-currency_symbol="true">{{ $contact_details->total_sell_return - $contact_details->total_return_paid - $contact_details->ledger_discount }}</span>
          </div>
        </div>
        @endif
      </div>
        ...
        <div class="col-md-4">
          <div class="form-group">
            ...
            <div class="input-group">
              ...
              {!! Form::text("amount",  @num_format($payment_line->amount - $contact_details->ledger_discount), ['class' => 'form-control input_number payment_amount', 'required', 'placeholder' => __('sale.amount'), 'data-rule-max-value' => $payment_line->amount, 'data-msg-max-value' => __('lang_v1.max_amount_to_be_paid_is', ['amount' => $amount_formated]), 'id' => 'due_payment_type_btn']); !!}
              @elseif ($due_payment_type == "sell")
                  {!! Form::text("amount",  @num_format(($payment_line->amount +  $contact_details->total_return_paid) - ( $contact_details->ledger_discount + $contact_details->total_sell_return)) , ['class' => 'form-control input_number payment_amount', 'required', 'placeholder' => __('sale.amount'), 'id' => 'due_payment_type_btn']); !!}
              @else
                {!! Form::text("amount", $due_payment_type == 'purchase' ? @num_format(($payment_line->amount +  $contact_details->total_return_paid) - ( $contact_details->ledger_discount + $contact_details->total_purchase_return)) :  @num_format($payment_line->amount - $contact_details->ledger_discount), ['class' => 'form-control input_number payment_amount', 'required', 'placeholder' => __('sale.amount'), 'id' => 'due_payment_type_btn']); !!}
              @endif
            </div>
          </div>
        </div>
