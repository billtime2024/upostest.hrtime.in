<tr class="product_row">
    <td>
        ...
        @if(in_array($product->item_type, ['imei', 'serial']))
            ...
            <br>
            <button type="button" class="btn btn-xs btn-primary select-imei-serial-btn" data-toggle="modal" data-target="#imei-serial-modal" data-row-index="{{$row_index}}" data-variation-id="{{$product->variation_id}}" data-item-type="{{$product->item_type}}">
    ...
    <td>
        ...
        @if(in_array($product->item_type, ['imei', 'serial']))
            ...
            <input type="text" class="form-control product_quantity input_number input_quantity imei-serial-qty" value="{{ in_array($product->item_type, ['imei', 'serial']) ? 0 : @format_quantity($product->quantity_ordered) }}" name="products[{{$row_index}}][quantity]" readonly data-rule-required="true" data-msg-required="@lang('validation.custom-messages.this_field_is_required')">
        @else
