        ...
        <tr>
            ...
            <td>
                ...
                @if(in_array($sell_line->product->item_type, ['imei', 'serial']))
                    @php
                        $identifiers = $sell_line->imei_numbers->pluck('identifier')->toArray();
                        $label = $sell_line->product->item_type == 'imei' ? 'IMEI' : 'Serial';
                    @endphp
                    @if(!empty($identifiers))
                        <br><strong>{{ $label }}:</strong> {{ implode(', ', $identifiers) }}
                    @endif
                @endif
            </td>
            @if( session()->get('business.enable_lot_number') == 1 && empty($for_ledger))
