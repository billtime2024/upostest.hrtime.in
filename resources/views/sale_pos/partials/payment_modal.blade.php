<div class="modal fade" tabindex="-1" role="dialog" id="modal_payment">
    ...
    onclick="function useAdvanceBalance(){
        const selectPayment = document.getElementById('method_0');
        if(selectPayment){
            const advanceAmountField = document.getElementById('amount_0');
            const advanceAmountValue = document.getElementById('advance_balance_text').innerText;
            const payableAmount = document.querySelector('.total_payable_span').innerText;
            const secondrowPayment = document.getElementById('amount_2');
           
            let newAdvanceAmountValue = '';
            let newPayableAmount = '';
            
            for(let i = 1; i < advanceAmountValue.length; i++){
                newAdvanceAmountValue += advanceAmountValue[i];
            }
            
            for(let i = 1; i < payableAmount.length; i++){
                newPayableAmount += payableAmount[i];
            }
            selectPayment.value = 'advance';
            console.log(parseFloat(newAdvanceAmountValue.split(',').join('')) > parseFloat(newPayableAmount.split(',').join('')));
            console.log(parseFloat(newAdvanceAmountValue.split(',').join('')),  parseFloat(newPayableAmount.split(',').join('')));
          if(parseFloat(newAdvanceAmountValue.split(',').join('')) > parseFloat(newPayableAmount.split(',').join(''))) {
                 advanceAmountField.value = newPayableAmount;
            }else{
                 advanceAmountField.value = newAdvanceAmountValue;
                // secondrowPayment.value = parseFloat(newPayableAmount.split(',').join('')) - parseFloat(newAdvanceAmountValue.split(',').join(''));
           }
        }
}; useAdvanceBalance();"
 type="button">Apply @lang('lang_v1.advance_balance')</button>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <button type="button" class="tw-dw-btn tw-dw-btn-primary tw-text-white tw-dw-btn-sm tw-w-full"
                                    id="add-payment-row"
                                    onclick="function addPaymentRow() {
                                     const selectPayment = document.getElementById('method_0');
        if(selectPayment && selectPayment.value === 'advance'){
            const advanceAmountField = document.getElementById('amount_0');
            const advanceAmountValue = document.getElementById('advance_balance_text').innerText;
            const payableAmount = document.querySelector('.total_payable_span').innerText;
          ...
          if(parseFloat(newAdvanceAmountValue.split(',').join('')) > parseFloat(newPayableAmount.split(',').join(''))) {
              //   advanceAmountField.value = newPayableAmount;
                ...
                // advanceAmountField.value = newAdvanceAmountValue;
                setTimeout(() => {
                 const secondrowPayment = document.getElementById('amount_1');
                 secondrowPayment.value = parseFloat(newPayableAmount.split(',').join('')) - parseFloat(newAdvanceAmountValue.split(',').join(''));
                 }, 1000);
           }
        }
                                    }; addPaymentRow();"
                                    >@lang('sale.add_payment_row')</button>
                            </div>
                        </div>
                        <br>
  ...
  <button id="loading_btn" style="display: none;" type="button" class="btn btn-primary" >
    ...
    <div style="display: none; background: black; opacity: 0.5;  position: absolute; top: 0; left: 0; width: 100%; height: 100%; justify-content:center; align-items: center; z-index: 99999;" id="loading">
...
<div class="modal fade" tabindex="-1" role="dialog" id="card_details_modal">
 ...
 <script>
        const newloadingModal = document.getElementById("loading");
        const newloadingBtn = document.getElementById("loading_btn");
        
        newloadingBtn.addEventListener("click", () => {
            newloadingModal.style.display ="flex";
        })
