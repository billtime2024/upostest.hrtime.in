...
if(auth()->user()->username !== "admin"){
    if(isset($default_location)){
        $locations = json_decode($default_location['default_payment_accounts']);
    }else {
        $locations = json_decode(\App\BusinessLocation::find($business_details["id"])['default_payment_accounts']);
    }
}

@endphp
@if(auth()->user()->username !== "admin"))
@if($custom_paymetns->custom_pay_1 !== null)

<button type="button" onclick="function createPos() {
const createPos = document.getElementById('pos-save')
const paymentAmount = document.getElementById('amount_0');
const paymentMethod = document.getElementById('method_0');
const account = document.getElementById('account_0');
const finalTotal = document.getElementById('final_total_input');
paymentAmount.value = finalTotal.value;
paymentMethod.value = 'custom_pay_1';
account.value = {{$locations->custom_pay_1->account}};

createPos.click();
}; createPos()" id="custom_pay_1"
                        class="tw-font-bold tw-text-gray-700 tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1  tw-text-white tw-bg-green-800  @if ($pos_settings['disable_draft'] != 0) hide @endif" style="padding: 0.75rem; border-radius: 0.73rem;"
                        data-pay_method="custom_pay_1" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                            Helloaria-hidden="true"></i> {{$custom_paymetns->custom_pay_1}}</button>
@endif

@if($custom_paymetns->custom_pay_2 !== null)
<button type="button" onclick="function createPos2() {
const createPos = document.getElementById('pos-save')
const paymentAmount = document.getElementById('amount_0');
const paymentMethod = document.getElementById('method_0');
const account = document.getElementById('account_0');
const finalTotal = document.getElementById('final_total_input');
paymentAmount.value = finalTotal.value;
paymentMethod.value = 'custom_pay_2';
account.value = {{ $locations->custom_pay_2->account}};

createPos.click();
}; createPos2()" id="custom_pay_2"
                        class="tw-font-bold tw-text-gray-700 tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1  tw-text-white tw-bg-sky-800 @if ($pos_settings['disable_draft'] != 0) hide @endif" style="padding: 0.75rem; border-radius: 0.73rem;"
                        data-pay_method="custom_pay_2" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                            Helloaria-hidden="true"></i> {{$custom_paymetns->custom_pay_2}}</button>
@endif

{{--
@if($custom_paymetns->custom_pay_3 !== null)
<button type="button"
                        ...
                        data-pay_method="custom_pay_3" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                            Helloaria-hidden="true"></i> {{$custom_paymetns->custom_pay_3}}</button>
@endif

@if($custom_paymetns->custom_pay_4 !== null)
<button type="button"
                        class="tw-font-bold tw-text-gray-700 tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1 @if ($pos_settings['disable_draft'] != 0) hide @endif"
                        data-pay_method="custom_pay_4" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                            Helloaria-hidden="true"></i> {{$custom_paymetns->custom_pay_4}}</button>
@endif

@if($custom_paymetns->custom_pay_5 !== null)
<button type="button"
                        class="tw-font-bold tw-text-gray-700 tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1 @if ($pos_settings['disable_draft'] != 0) hide @endif"
                        data-pay_method="custom_pay_5" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                            Helloaria-hidden="true"></i> {{$custom_paymetns->custom_pay_5}}</button>
@endif

@if($custom_paymetns->custom_pay_6 !== null)
<button type="button"
                        class="tw-font-bold tw-text-gray-700 tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1 @if ($pos_settings['disable_draft'] != 0) hide @endif"
                        data-pay_method="custom_pay_6" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                            Helloaria-hidden="true"></i> {{$custom_paymetns->custom_pay_6}}</button>
@endif

@if($custom_paymetns->custom_pay_7 !== null)
<button type="button"
                        class="tw-font-bold tw-text-gray-700 tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1 @if ($pos_settings['disable_draft'] != 0) hide @endif"
                        data-pay_method="custom_pay_7" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                            Helloaria-hidden="true"></i> {{$custom_paymetns->custom_pay_7}}</button>
@endif
--}}
@endif
                @if (empty($edit))
...
createPos.click();
                        ...
                        class="tw-hidden md:tw-flex md:tw-flex-row md:tw-items-center md:tw-justify-center md:tw-gap-1 tw-font-bold tw-font-bold tw-text-gray-700 tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1  tw-text-white tw-bg-green-800 @if (!$is_mobile)  @endif no-print  @if ($pos_settings['disable_draft'] != 0) hide @endif" style="padding: 0.75rem; border-radius: 0.73rem; text-transform: capitalize; "
                        data-pay_method="custom_pay_1" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                            Helloaria-hidden="true"></i> {{strtolower($custom_paymetns->custom_pay_1)}}</button>
@endif
...
createPos.click();
                        ...
                        class="tw-hidden md:tw-flex md:tw-flex-row md:tw-items-center md:tw-justify-center md:tw-gap-1 tw-font-bold tw-font-bold tw-text-gray-700 tw-text-xs md:tw-text-sm tw-flex tw-flex-col tw-items-center tw-justify-center tw-gap-1  tw-text-white tw-bg-sky-800 @if (!$is_mobile)  @endif no-print @if ($pos_settings['disable_draft'] != 0) hide @endif" style="padding: 0.75rem; border-radius: 0.73rem; text-transform: capitalize;"
                        data-pay_method="custom_pay_2" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt"
                            Helloaria-hidden="true"></i> {{strtolower($custom_paymetns->custom_pay_2)}}</button>
@endif
