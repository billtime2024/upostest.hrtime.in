<div class="row">
	...
	@php
		...
		//dd(gettype($payment_types), $payment_types);

		// Debug logging for undefined variables
		if (!isset($payment_types)) {
		    error_log('payment_types not set in payment_row_form.blade.php');
		}
		if (!isset($payment_line)) {
		    error_log('payment_line not set in payment_row_form.blade.php');
		}
		if (!isset($accounts)) {
		    error_log('accounts not set in payment_row_form.blade.php');
		}
		if (!isset($show_date)) {
		    error_log('show_date not set in payment_row_form.blade.php');
		}
		if (!isset($show_denomination)) {
		    error_log('show_denomination not set in payment_row_form.blade.php');
		}
	@endphp
	<div class="{{$col_class}}">
	...
	<div class="{{$col_class}}">
		<div class="form-group">
			...
			<div class="input-group">
				...
				//	dd($_payment_method, $payment_line["method"]);
				@endphp
				{!! Form::select("payment[$row_index][method]", $payment_types, $_payment_method, ['class' => 'form-control col-md-12 payment_types_dropdown', 'required', 'id' => !$readonly ? "method_$row_index" : "method_advance_$row_index", 'style' => 'width:100%;', 'disabled' => $readonly, 'required']); !!}

				@if($readonly)
					{!! Form::hidden("payment[$row_index][method]", $payment_line['method'], ['class' => 'payment_types_dropdown',  'id' => "method_$row_index"]); !!}
				@endif
			</div>
		</div>
	</div>
...
function start () {
    if( window.location.href.split("/")[window.location.href.split("/").length - 2] !== "pos" &&  window.location.href.split("/")[window.location.href.split("/").length - 3] !== "pos") {
        const paymentDropDowns =[...document.querySelectorAll(".payment_types_dropdown > option")];
        paymentDropDowns.forEach(paymentDropDown => {
            if(paymentDropDown.value != "none") {
                paymentDropDown.setAttribute("selected", "false");
            } else {
                paymentDropDown.setAttribute("selected", "selected");
            }
        })
    }
}
