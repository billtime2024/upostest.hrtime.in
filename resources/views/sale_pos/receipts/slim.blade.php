...
<div
    style="width: 68 mm; page-break:avoid; font-family:'Bree Serif', 'verdana'; font-size:8pt; line-height:16px; margin: 1rem; padding:5px; z-index:'9999'">
    <!-- Logo-->
    @if (!empty($receipt_details->logo))
        <left><img src="{{ $receipt_details->logo }}" style="width:20%;"></center>
    @endif
              		<center><td>Tax Invoice</td>
    <div style="text-align: center; line-height:13px">
        ...
        <table style="width:100%">
            <tr>
                <td style="width:50%; text-align:left; font-size:7pt">
			...
			<tr style=""><td style="font-size:7pt; vertical-align:top;" colspan="4">
              ...
              @endif
                  ...
                  @if($line['item_type'] == 'imei' || $line['item_type'] == 'serial')
                      @if($line['item_type'] == 'imei')
                          IMEI: {{ implode(', ', $line['sold_imei'] ?? []) }}<br>
                      @else
                          Serial: {{ implode(', ', $line['sold_serial'] ?? []) }}<br>
                      @endif
                  @endif
                </td></tr>
    ...
    <table style="width: 100%; font-size:6pt; font-weight:bold">
        ...
        @if (!empty($receipt_details->round_off_label))
            <tr>
                ...
                <td style="text-align: right; font-size:8pt">
                    ...
                    @php
                    if(isset($receipt_details->round_off_uf)){
                        $roundoff = $receipt_details->round_off_uf;
                        }else {
                        $roundoff = $receipt_details->round_off;
                        }
                    @endphp
                </td>
            </tr>
       ...
       @endif
        ...
        @endif
                ...
                @if($total_discount != 0 )
                    <hr style='margin:5px'>
              		<div style="text-align: left;">You saved <span style='font-weight:bold; font-size: 0.75rem; text-align: left; padding: 1rem;' class='display_currency' data-currency_symbol='true'> {{$total_discount}} </span> </div>
                    @endif
    <hr style="margin:5px">
    <center>
                ...
                @endphp
					...
					<div >
						...
						<div class="text-right">
						    Earned: {{$receipt_details->reward_point_earned}}</br>
							Total: {{$receipt_details->reward_point_amount}}
						</div>
					</div>
				@endif
                <hr style="margin:0px">
