...
@section('content')
    ...
    <div class="modal fade" id="stock-details-modal" tabindex="-1" role="dialog" aria-labelledby="stockDetailsModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="stockDetailsModalLabel">Available Stocks</h4>
...
<!--    </div>-->
<!--    <div style="background: transparent !important;" class="modal fade" tabindex="-1" role="dialog" id="loading">-->
<!--    <div class="modal-dialog modal-lg" role="document">-->
<!--        <div class="modal-content">-->
<!--            <div class="modal-header">-->
<!--                <button style="display: none;" id="loading_close" type="button" class="close" data-dismiss="modal" aria-label="Close"><span-->
<!--                        aria-hidden="true">&times;</span></button>-->
<!--                <h4 class="modal-title"></h4>-->
<!--            </div>-->
<!--            <div class="modal-body" style="display: flex; justify-content: center; align-items: center;">-->
<!--                <img src="{{ asset('images/loading/bill_loading.gif') }}" style="width: 30vw; height: 40vh;" alt="loading" >-->
<!--            </div>-->
<!--            <div class="modal-footer">-->
...
@section('javascript')
    ...
    <script>
        ...
        $(document).on('click', '.select-imei-serial-btn', function() {
            ...
            // Get existing selected IMEI/serial values for this row
            var existingSelections = [];
            $('input[name="products[' + rowIndex + '][selected_imei_serial][]"]').each(function() {
                existingSelections.push($(this).val());
            });
                ...
                success: function(response) {
                    ...
                    if (response.success && response.data.length > 0) {
                        response.data.forEach(function(item) {
                            var checked = existingSelections.indexOf(item.identifier) !== -1 ? 'checked' : '';
        ...
        $(document).on('input', '.imei-serial-search', function() {
            ...
            // Auto-select if only one visible
            var visibleCheckboxes = modal.find('.imei-serial-checkbox').filter(function() {
                return $(this).closest('.checkbox').is(':visible');
            });
        ...
        // Mutual exclusivity between lot number and IMEI/serial selection
        $(document).on('change', '.lot-imei-selector.lot_number', function() {
            var rowIndex = $(this).data('row-index');
            var selectedValue = $(this).val();

            if (selectedValue !== '') {
                // Disable IMEI/Serial button if lot is selected
                $('.select-imei-serial-btn[data-row-index="' + rowIndex + '"]').prop('disabled', true).addClass('disabled');
                // Clear any selected IMEI/serial values
                $('input[name="products[' + rowIndex + '][selected_imei_serial][]"]').remove();
                // Reset quantity to 1 if it was set by IMEI/serial selection
                var quantityField = $('input[name="products[' + rowIndex + '][quantity]"]');
                if (quantityField.val() > 1) {
                    quantityField.val(1).trigger('change');
                }
            } else {
                // Re-enable IMEI/Serial button if lot is deselected
                $('.select-imei-serial-btn[data-row-index="' + rowIndex + '"]').prop('disabled', false).removeClass('disabled');
            }
        });

        // Handle IMEI/Serial button click - disable lot selection
        $(document).on('click', '.select-imei-serial-btn.lot-imei-selector', function() {
            var rowIndex = $(this).data('row-index');
            // Clear lot selection
            $('.lot_number[data-row-index="' + rowIndex + '"]').val('').trigger('change');
        });
    </script>
    <script>
        ...
        $(document).on('click', '.last-purchase-price', function() {
            var variation_id = $(this).data('variation-id');
            var location_id = $('#location_id').val();
            $.ajax({
                url: '/sell-pos/get-stock-details/' + variation_id,
                method: 'GET',
                data: { location_id: location_id },
                success: function(response) {
                    $('#stock-details-modal .modal-body').html(response.html);
