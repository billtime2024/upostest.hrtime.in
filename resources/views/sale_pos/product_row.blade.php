...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	@if(!empty($is_serial_no))
		<td class="serial_no" ><input type="text" class="form-control serial_number" name="serial_number[]"></td>
	@endif
	<td>
		@if(!empty($so_line))
			<input type="hidden" 
		...
		@endif
		@php
			$product_name = "<div><span style='color: #0070C0; font-weight: bold;'>" . $product->product_name . '</span> - <span style="color: #7030A0">' . $product->sub_sku . "</span>" ;
			if(!empty($product->brand)){ 
			    $product_name .= '<span style="color: #00B050;">' .' ' . $product->brand . '</span>' ;
			}
...
<br>
		<small class="text-muted p-1">
			@if($product->enable_stock)
		          {{ @num_format($product->qty_available) }} {{$product->unit}} @lang('lang_v1.in_stock') @can ("view_purchase_price")<span class="last-purchase-price" data-variation-id="{{ $product->variation_id }}" style="cursor: pointer;">Last Purchase Price: {{ @num_format($product->last_purchased_price) }}</span>@endcan
			@else
				--
			@endif
		</small>
		...
		@if(!empty($product->lot_numbers) && empty($is_sales_order))
			<select style="color: #000 !important;" class="form-control lot_number input-sm lot-imei-selector" data-row-index="{{$row_count}}" data-selector-type="lot" name="products[{{$row_count}}][lot_no_line_id]" @if(!empty($product->transaction_sell_lines_id)) disabled @endif >
	...
	@if(in_array($product->item_type, ['imei', 'serial']))
		<br>
		<button type="button" class="btn btn-sm btn-primary select-imei-serial-btn lot-imei-selector" data-selector-type="imei" data-toggle="modal" data-target="#imei-serial-modal" data-row-index="{{$row_count}}" data-variation-id="{{$product->variation_id}}" data-item-type="{{$product->item_type}}" @if(!empty($product->transaction_sell_lines_id)) data-sell-line-id="{{$product->transaction_sell_lines_id}}" @endif>@lang('Select Serial/IMEI')</button>
	@endif
</td>
	...
	<td>
		...
		@foreach($sub_units as $key => $value)
        	...
        	@endif
        @endforeach
        <div style="display: flex; justify-content: space-around; align-items: center; width:  20vw; ">
		...
		<input type="text" data-min="1"
			...
			@if(in_array($product->item_type, ['imei', 'serial'])) readonly @endif
		...
		@if(count($sub_units) > 0)
			<!--<br>-->
			<select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit" style="width: 5vw;">
                @foreach($sub_units as $key => $value)
...
@php
    $new_service_staff = null;

    if(isset($product->res_service_staff_id) && !empty($product->res_service_staff_id) ) {
        $new_service_staff =  $product->res_service_staff_id;
    } else if( $so_line !== null && $so_line->res_service_staff_id !== null) {
        $new_service_staff = $so_line->res_service_staff_id;
    } else {
        $new_service_staff = null;
    }
			...
			<td>
				<div class="form-group">
					<div class="input-group">
						{!! Form::select("products[" . $row_count . "][res_service_staff_id]", $waiters, $new_service_staff, ['class' => 'form-control select2 order_line_service_staff', 'placeholder' => __('restaurant.select_service_staff'), 'required' => (!empty($pos_settings['is_service_staff_required']) && $pos_settings['is_service_staff_required'] == 1) ? true : false ]); !!}
					</div>
				</div>
			</td>
		@endif
