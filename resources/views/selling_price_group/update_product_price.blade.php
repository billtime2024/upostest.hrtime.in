...
<section class="content" style="min-height: 100vh;">
    ...
    @component('components.widget', ['class' => 'box-primary', 'title' => __('lang_v1.import_export_product_price')])
            <div class="row">
                ...
                <div class="col-sm-6">
                    {!! Form::open(['url' => action([\App\Http\Controllers\SellingPriceGroupController::class, 'import']), 'method' => 'post', 'enctype' => 'multipart/form-data', 'id' => 'update_product_price_form' ]) !!}
                    <div class="form-group">
                        {!! Form::label('name', __( 'product.file_to_import' ) . ':') !!}
                        {!! Form::file('product_group_prices', ['required' => 'required']); !!}
                    </div>
                    <div class="form-group">
                        <button id="update_product_price" type="submit" class="tw-dw-btn tw-dw-btn-primary tw-text-white">@lang('messages.submit')</button>
                    </div>
                    {!! Form::close() !!}
                </div>
...
@section('javascript')
<script>
    const updatePriceBtn = document.getElementById("update_product_price");
    const updatePriceForm = document.getElementById("update_product_price_form");
    
    updatePriceBtn.addEventListener("click", (e) => {
        e.preventDefault();
