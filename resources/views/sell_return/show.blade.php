        ...
        <tbody>
            @php
              ...
              foreach($sell->sell_lines as $sell_line){
                //dd($sell_line);
                $line_total = (float) $sell_line->unit_price * $sell_line->quantity_returned;
                $tax = \App\TaxRate::where("id", $sell_line["tax_id"])->get()[0];
                $total_tax += ((float) $tax["amount"] / 100 ) * $line_total;
              }
            @endphp
            @foreach($sell->sell_lines as $sell_line)
