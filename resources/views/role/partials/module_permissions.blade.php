 @if(count($module_permissions) > 0)
  @php
    ...
    $package_details = \App\Business::find(auth()->user()->business_id)->subscriptions[0]->package_details;
    if(!empty($role_permissions)) {
      $module_role_permissions = $role_permissions;
    }
  @endphp
  @foreach($module_permissions as $key => $value)
  @if( auth()->user()->id !== 1 && $key !== "Superadmin")
    @if(isset($package_details[strtolower($key) . "_module"]) && $package_details[strtolower($key) . "_module"] == "1" )
  <hr>
