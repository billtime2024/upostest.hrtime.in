...
<section class="content-header">
   	<h1 class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">
   		...
   		<span id="draft_indicator" class="label label-warning" style="display: none;">@lang('lang_v1.draft')</span>
   	...
   	<div id="draft_status" class="alert alert-info" style="display: none;">
   		<i class="fa fa-save"></i> <span id="draft_status_text"></span>
   	</div>
</section>
...
<section class="content">
		...
		<div class="row">
			...
			<div class="col-sm-3">
				<div class="form-group">
					...
					{!! Form::select('location_id', $business_locations, $purchase->location_id ?? $default_location, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required'], $bl_attributes); !!}
				</div>
			</div>
				...
				<tr>
					<td colspan="4">
						<div class="form-group">
							...
							{!! Form::textarea('additional_notes', $purchase->additional_notes ?? null, ['class' => 'form-control', 'rows' => 3]); !!}
						</div>
					</td>
				</tr>
	...
	<div class="row">
			<div class="col-sm-12 text-center">
				<button type="button" id="save_as_draft" class="tw-dw-btn tw-dw-btn-warning tw-dw-btn-lg tw-text-white" style="margin-right: 10px;">@lang('lang_v1.save_as_draft')</button>
...
@section('javascript')
	...
	<script type="text/javascript">
		...
		function saveDraft(auto_save = false) {
			var form_data = new FormData($('#add_purchase_form')[0]);
			form_data.append('_token', $('meta[name="csrf-token"]').attr('content'));

			$.ajax({
				url: '{{ route("purchaseOrder.storeDraft") }}',
				type: 'POST',
				data: form_data,
				processData: false,
				contentType: false,
				success: function(response) {
					if (response.success) {
						$('#draft_id').val(response.transaction_id);
						$('#draft_indicator').show();
						is_draft_mode = true;

						var now = new Date();
						var time_str = now.getHours().toString().padStart(2, '0') + ':' + now.getMinutes().toString().padStart(2, '0');
						$('#draft_status_text').text('@lang("lang_v1.draft_saved_at") ' + time_str);
						$('#draft_status').show();
		...
		$(document).ready( function(){
			...
			// Check if draft_id is set (resuming draft)
			if ($('#draft_id').val() != '') {
				$('#draft_indicator').show();
				is_draft_mode = true;
				startAutoSave();
			...
			// Populate products if resuming draft
			@if($purchase && $purchase->purchase_lines)
				@php
					$draft_lines = $purchase->purchase_lines->map(function($line) {
						$line->mfg_date_formatted = $line->mfg_date ? \Carbon\Carbon::parse($line->mfg_date)->format('m/d/Y') : '';
						$line->exp_date_formatted = $line->exp_date ? \Carbon\Carbon::parse($line->exp_date)->format('m/d/Y') : '';
						return $line;
					});
				@endphp
				var draft_lines = {!! json_encode($draft_lines) !!};
				var current_index = 0;

				function add_next_row() {
					if (current_index < draft_lines.length) {
						var line = draft_lines[current_index];
						get_purchase_entry_row(line.product_id, line.variation_id);
						// Wait for the row to be added, then set values
						setTimeout(function() {
							var row = $('#purchase_entry_table tbody tr').eq(current_index);
							row.find('.purchase_quantity').val(line.quantity);
							row.find('.purchase_unit_cost').val(line.purchase_price);
							row.find('.inline_discounts').val(line.discount_percent || 0);
							row.find('select.purchase_line_tax_id').val(line.tax_id || '');
							row.find('textarea[name*="lot_number"]').val(line.lot_number || '');
							if (line.mfg_date_formatted) {
								row.find('.mfg_date').val(line.mfg_date_formatted);
							}
							if (line.exp_date_formatted) {
								row.find('.exp_date').val(line.exp_date_formatted);
							}
			...
			// Load supplier address if resuming draft
			@if($purchase && $purchase->contact_id)
				setTimeout(function() {
			...
			// Start auto-save when form changes for the first time
			$('#add_purchase_form').on('change input', ':input', function() {
				if (!is_draft_mode && $('#draft_id').val() == '') {
					// First change, enable draft mode
					is_draft_mode = true;
					startAutoSave();
				}
			});
