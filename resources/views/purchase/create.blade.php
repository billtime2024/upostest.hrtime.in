...
@section('content')
<style>
	.mismatch-highlight .purchase_quantity,
	.mismatch-highlight [name*='[lot_number]'] {
	background-color: #f8d7da; /* Light red */
	border: 1px solid #f5c2c7; /* Red border */
	}
	.duplicate-lot-highlight {
	border: 2px solid red; /* or any other highlight style */
	background-color: #ffcccc;
	
	}
     textarea[name^="purchases"][name*="[lot_number]"] {
      width: 150px; 
      height: 70px; 
      }
</style>
			...
			<div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
				<div class="form-group">
					...
					<div class="input-group">
						...
						{!! Form::select('contact_id', [], old('contact_id') , ['class' => 'form-control', 'placeholder' => __('messages.please_select'), 'required', 'id' => 'supplier_id']); !!}
...
@php
    $contact = [];
    if(count($old) > 0){
        $contact = \App\Contact::where('id', $old['contact_id'])->get()[0];
    }
@endphp
            </div>
				<strong>
			...
			<div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
				<div class="form-group">
					...
					{!! Form::text('ref_no', old('ref_no'), ['class' => 'form-control']); !!}
				</div>
			</div>
			<div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
				<div class="form-group">
			...
			<div class="col-sm-3 @if(!empty($default_purchase_status)) hide @endif">
				<div class="form-group">
					...
					{!! Form::select('status', $orderStatuses, old('status') ? old('status') : $default_purchase_status, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required']); !!}
				</div>
			</div>			
			...
			<div class="col-sm-3">
				<div class="form-group">
					...
					{!! Form::select('location_id',  $business_locations, old('location_id') ? old('location_id') :$default_location, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required'], $bl_attributes); !!}
				</div>
			</div>
<div class="col-sm-2">
	<div class="form-group">
		{!! Form::label('lot_type', __('Lot Number Type') . ':*') !!}
		{!! Form::select('lot_type', ['single' => 'Single lot', 'multiple' => 'Multiple lots'], old('lot_type') ? old('lot_type') :'single', [
			...
			<div class="col-md-3">
		          <div class="form-group">
		            <div class="multi-input">
		              ...
		              <br/>
		              {!! Form::number('pay_term_number', old('pay_term_number'), ['class' => 'form-control width-40 pull-left', 'min' => 0, 'placeholder' => __('contact.pay_term')]); !!}
   ...
   <!--         </div>-->
		...
		@if(!empty($custom_field_1_label))
			...
			<div class="col-md-4">
		        <div class="form-group">
		            ...
		            {!! Form::text('custom_field_1', old('custom_field_1'), ['class' => 'form-control','placeholder' => $custom_field_1_label, 'required' => $is_custom_field_1_required]); !!}
		        </div>
		    </div>
		@endif
		@if(!empty($custom_field_2_label))
			...
			<div class="col-md-4">
		        <div class="form-group">
		            ...
		            {!! Form::text('custom_field_2', old('custom_field_2'), ['class' => 'form-control','placeholder' => $custom_field_2_label, 'required' => $is_custom_field_2_required]); !!}
		        </div>
		    </div>
		@endif
		@if(!empty($custom_field_3_label))
			...
			<div class="col-md-4">
		        <div class="form-group">
		            ...
		            {!! Form::text('custom_field_3', old('custom_field_3'), ['class' => 'form-control','placeholder' => $custom_field_3_label, 'required' => $is_custom_field_3_required]); !!}
		        </div>
		    </div>
		@endif
		@if(!empty($custom_field_4_label))
			...
			<div class="col-md-4">
		        <div class="form-group">
		            ...
		            {!! Form::text('custom_field_4', old('custom_field_4'), ['class' => 'form-control','placeholder' => $custom_field_4_label, 'required' => $is_custom_field_4_required]); !!}
		        </div>
		    </div>
		@endif
		</div>
		...
		<div class="row">
			<div class="col-sm-3">
				<div class="form-group">
					...
					{!! Form::select('purchase_order_ids[]', [], old('purchase_order_ids[]'), ['class' => 'form-control select2', 'multiple', 'id' => 'purchase_order_ids']); !!}
				</div>
			</div>
		</div>
		@endif
	...
	@component('components.widget', ['class' => 'box-primary'])
			...
			<div class="col-sm-12">
				<div class="table-responsive">
					<table class="table table-condensed table-bordered table-th-green text-center table-striped" id="purchase_entry_table">
                        ...
                        <tbody>
                            @if (count($old) > 0 && count($old["purchases"]) > 0)
                            @foreach ($old["purchases"] as $key => $purchase)
                            @php
                                $product = \App\Product::where('id',  (int) $purchase["product_id"])->get()[0];
                                $variation = \App\Variation::where('id',  (int) $purchase["variation_id"])->get()[0];
                            @endphp
                                <tr>
        <td><span class="sr_number">{{$key}}</span></td>
        <td>
            {{$product["name"]}} ({{$variation["sub_sku"]}})
                            <br>
                (<b>{{$product["name"]}}</b> : {{$variation["name"]}})
                                        <br>
                <small class="text-muted" style="white-space: nowrap;">Current stock:  3.00  Pc(s)</small>
                        
        </td>
        <td>
            
            
            <input name="purchases[{{$key}}][product_id]" type="hidden" value="{{$purchase['product_id']}}">
            <input class="hidden_variation_id" name="purchases[{{$key}}][variation_id]" type="hidden" value="{{$purchase['variation_id']}}">

                        
            <input type="text" name="purchases[{{$key}}][quantity]" value="{{$purchase['quantity']}}" class="form-control input-sm purchase_quantity input_number mousetrap" required="" data-rule-abs_digit="true" data-msg-abs_digit="Decimal value not allowed">


            <input type="hidden" class="base_unit_cost" value="{{$purchase['purchase_price']}}">
            <input type="hidden" class="base_unit_selling_price" value="{{$purchase['default_sell_price']}}">

            <input type="hidden" name="purchases[{{$key}}][product_unit_id]" value="{{$purchase['product_unit_id']}}">
                            <br>
                <select name="purchases[{{$key}}][sub_unit_id]" class="form-control input-sm sub_unit">
                                            <option value="{{$purchase['sub_unit_id']}}" data-multiplier="1">
                            Pieces
                        </option>
                                    </select>
        ...
        <td>
                        <input class="form-control input-sm purchase_unit_cost_without_discount input_number" required="" name="purchases[{{$key}}][pp_without_discount]" type="text" value="{{$purchase['pp_without_discount']}}">

                            <br>
                <small class="text-muted">Previous unit price: ₹ {{$purchase['pp_without_discount']}}</small>
                    </td>
        <td>
            <input class="form-control input-sm inline_discounts input_number" required="" name="purchases[{{$key}}][discount_percent]" type="text" value="{{$purchase['discount_percent']}}">
        ...
        <td>
            <input class="form-control input-sm purchase_unit_cost input_number" required="" name="purchases[{{$key}}][purchase_price]" type="text" value="{{$purchase['purchase_price']}}">
        </td>
        <td class="">
            <span class="row_subtotal_before_tax display_currency">{{$old['total_before_tax']}}</span>
            <input type="hidden" class="row_subtotal_before_tax_hidden" value="{{$old['total_before_tax']}}">
        </td>
        <td class="">
            <div class="input-group">
                <select name="purchases[{{$key}}][purchase_line_tax_id]" value="{{$purchase['purchase_line_tax_id'] === null ? '' : $purchase['purchase_line_tax_id']}}" class="form-control select2 input-sm purchase_line_tax_id" placeholder="'Please Select'">
                    <option value="" data-tax_amount="0">None</option>
                                            <option value="43" data-tax_amount="{{$purchase['purchase_line_tax_id']}}">GST {{$purchase['purchase_line_tax_id']}}</option>
                                    </select>
                <input class="purchase_product_unit_tax" name="purchases[{{$key}}][item_tax]" type="hidden" value="{{$purchase['item_tax']}}">
                <span class="input-group-addon purchase_product_unit_tax_text">{{$purchase['item_tax']}}</span>
            </div>
        </td>
        <td class="">
                        <input class="form-control input-sm purchase_unit_cost_after_tax input_number" required="" name="purchases[{{$key}}][purchase_price_inc_tax]" type="text" value="{{$purchase['purchase_price_inc_tax']}}">
        </td>
        <td>
            <span class="row_subtotal_after_tax display_currency">{{$purchase['purchase_price_inc_tax']}}</span>
            <input type="hidden" class="row_subtotal_after_tax_hidden" value="{{$old['final_total']}}">
        </td>
        <td class="">
            <input class="form-control input-sm input_number profit_percent" required="" name="purchases[{{$key}}][profit_percent]" type="text" value="{{$purchase['profit_percent']}}">
        </td>
                <td>
                                        <input class="form-control input-sm input_number default_sell_price" required="" name="purchases[{{$key}}][default_sell_price]" type="text" value="{{$purchase['default_sell_price']}}">
                    </td>
                                <td>
                <textarea class="form-control input-sm" name="purchases[{{$key}}][lot_number]" cols="50" rows="10">{{$purchase['lot_number']}}</textarea>

            </td>
                                <td><i class="fa fa-recycle auto_generate_lot_row text-success" title="generate lot number" style="cursor:pointer;"></i></td>
			...
			<table class="table">
				<tr>
					...
					<td class="col-md-3">
						...
						{!! Form::text('discount_amount', old('discount_amount')? old('discount_amount') :0, ['class' => 'form-control input_number', 'required']); !!}
						</div>
					</td>
					<td class="col-md-3">
						&nbsp;
				...
				<tr>
					<td>
						...
						<select name="tax_id" id="tax_id" class="form-control select2" placeholder="'Please Select'">
							<option value="" data-tax_amount="0" data-tax_type="fixed" {{old('tax_id') ? '' :'selected'}}>@lang('lang_v1.none')</option>
							@foreach($taxes as $tax)
								<option  {{old('tax_id') == $tax->id ? 'selected' : '' }} value="{{ $tax->id }}" data-tax_amount="{{ $tax->amount }}" data-tax_type="{{ $tax->calculation_type }}">{{ $tax->name }}</option>
							@endforeach
						</select>
						{!! Form::hidden('tax_amount', old('tax_amount') ? old('tax_amount') : 0, ['id' => 'tax_amount']); !!}
						</div>
					</td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
				...
				<tr>
					<td colspan="4">
						<div class="form-group">
							...
							{!! Form::textarea('additional_notes', old('additional_notes'), ['class' => 'form-control', 'rows' => 3]); !!}
						</div>
					</td>
				</tr>
	...
	<div class="row">
		<div class="col-md-4">
			...
			{!! Form::text('shipping_details', old('shipping_details'), ['class' => 'form-control']); !!}
			</div>
		</div>
		<div class="col-md-4 col-md-offset-4">
			<div class="form-group">
				{!! Form::label('shipping_charges','(+) ' . __( 'purchase.additional_shipping_charges' ) . ':') !!}
				{!! Form::text('shipping_charges', old('shipping_charges') ? old('shipping_charges') : 0, ['class' => 'form-control input_number', 'required']); !!}
			</div>
		</div>
	</div>
	<div class="row">
			...
			@if(!empty($shipping_custom_label_1))
				...
				<div class="col-md-4">
			        <div class="form-group">
			            ...
			            {!! Form::text('shipping_custom_field_1', old('shipping_custom_field_1'), ['class' => 'form-control','placeholder' => $shipping_custom_label_1, 'required' => $is_shipping_custom_field_1_required]); !!}
			        </div>
			    </div>
			@endif
			@if(!empty($shipping_custom_label_2))
				...
				<div class="col-md-4">
			        <div class="form-group">
			            ...
			            {!! Form::text('shipping_custom_field_2', old('shipping_custom_field_2'), ['class' => 'form-control','placeholder' => $shipping_custom_label_2, 'required' => $is_shipping_custom_field_2_required]); !!}
			        </div>
			    </div>
			@endif
			@if(!empty($shipping_custom_label_3))
				...
				<div class="col-md-4">
			        <div class="form-group">
			            ...
			            {!! Form::text('shipping_custom_field_3', old('shipping_custom_field_3'), ['class' => 'form-control','placeholder' => $shipping_custom_label_3, 'required' => $is_shipping_custom_field_3_required]); !!}
			        </div>
			    </div>
			@endif
			@if(!empty($shipping_custom_label_4))
				...
				<div class="col-md-4">
			        <div class="form-group">
			            ...
			            {!! Form::text('shipping_custom_field_4', old('shipping_custom_field_4'), ['class' => 'form-control','placeholder' => $shipping_custom_label_4, 'required' => $is_shipping_custom_field_4_required]); !!}
			        </div>
			    </div>
			@endif
			@if(!empty($shipping_custom_label_5))
				...
				<div class="col-md-4">
			        <div class="form-group">
			            ...
			            {!! Form::text('shipping_custom_field_5', old('shipping_custom_field_5'), ['class' => 'form-control','placeholder' => $shipping_custom_label_5, 'required' => $is_shipping_custom_field_5_required]); !!}
			        </div>
			    </div>
			@endif
		</div>
...
@include('purchase.partials.imei_manager_modal')
@include('purchase.partials.serial_manager_modal')
...
@section('javascript')
    ...
    @if (count($old) > 0)
        setTimeout(() => {
            const supplierIdEle = document.getElementById('select2-supplier_id-container');
            const supplierIdSelect = document.getElementById("supplier_id");
            const supplierOption = document.createElement("option");
            supplierIdEle.value = "{{$contact['name']}}";
            supplierIdEle.innerText = "{{$contact['name']}}";
            supplierOption.value =  "{{old('contact_id')}}";
            supplierOption.innerText =  "{{$contact['name']}}";
            supplierOption.selected = true;
            supplierIdSelect.appendChild(supplierOption);
            supplierIdSelect.setAttribute("value","{{old('contact_id')}}");
        }, 500)
@endif
