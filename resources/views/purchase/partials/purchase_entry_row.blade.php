    ...
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        <td>
            ...
            @if($product->item_type == 'imei')
                <br>
                <button type="button" class="btn btn-sm btn-primary add-imei-btn" data-row="{{ $row_count }}">
                    <i class="fa fa-plus"></i> @lang('Add IMEI')
                </button>
            @endif
            @if($product->item_type == 'serial')
                <br>
                <button type="button" class="btn btn-sm btn-primary add-serial-btn" data-row="{{ $row_count }}">
                    <i class="fa fa-plus"></i> @lang('Add Serial')
                </button>
            @endif
        ...
        <td>
            ...
            @php
                ...
                $max_quantity_message = __('lang_v1.max_quantity_quantity_allowed', ['quantity' => $max_quantity]);
            @endphp
            ...
            <input type="text"
                ...
                required
                @if($product->item_type == 'imei' || $product->item_type == 'serial') readonly @endif
                data-rule-abs_digit={{$check_decimal}}
                data-msg_abs_digit="{{__('lang_v1.decimal_value_not_allowed')}}"
                @if(!empty($max_quantity))
                    data-rule-max-value="{{$max_quantity}}"
                    data-msg-max-value="{{$max_quantity_message}}"
                @endif
            >
        ...
        <td>
            @php
            $sell_price_inc_tax = number_format($variation->sell_price_inc_tax, $currency_precision, $currency_details->decimal_separator,  $currency_details->thousand_separator);

            $sell_price_inc_tax = !empty($imported_data['sell_price_inc_tax']) ? $imported_data['sell_price_inc_tax'] : $sell_price_inc_tax;
            @endphp
            @if(session('business.enable_editing_product_from_purchase'))
                {!! Form::text('purchases[' . $row_count . '][default_sell_price]',$sell_price_inc_tax , ['class' => 'form-control input-sm input_number default_sell_price', 'required']); !!}
            @else
        ...
        @if(session('business.enable_lot_number') && $product->item_type != 'imei' && $product->item_type != 'serial')
            @php
                $lot_number = !empty($imported_data['lot_number']) ? $imported_data['lot_number'] : null;
            @endphp
            <td>
                {!! Form::textarea('purchases[' . $row_count . '][lot_number]', $lot_number, ['class' => 'form-control input-sm']); !!}
        ...
        @endif
        <?php $row_count++ ;?>
<td><i class="fa fa-recycle auto_generate_lot_row text-success" title="generate lot number"
