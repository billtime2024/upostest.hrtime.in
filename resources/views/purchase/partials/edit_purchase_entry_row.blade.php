    ...
    @foreach($purchase->purchase_lines as $purchase_line)
        <tr @if(!empty($purchase_line->purchase_order_line) && !empty($common_settings['enable_purchase_order'])) data-purchase_order_id="{{$purchase_line->purchase_order_line->transaction_id}}" @endif  @if(!empty($purchase_line->purchase_requisition_line) && !empty($common_settings['enable_purchase_requisition'])) data-purchase_requisition_id="{{$purchase_line->purchase_requisition_line->transaction_id}}" @endif>
            ...
            <td>
                ...
                @endif
                @if($purchase_line->product->item_type == 'imei')
                    <br>
                    <button type="button" class="btn btn-sm btn-primary manage-imei-btn" data-row="{{ $loop->index }}" data-purchase-line-id="{{ $purchase_line->id }}">
                        <i class="fa fa-cogs"></i> @lang('Manage IMEI')
                    </button>
                @endif
                @if($purchase_line->product->item_type == 'serial')
                    <br>
                    <button type="button" class="btn btn-sm btn-primary manage-serial-btn" data-row="{{ $loop->index }}" data-purchase-line-id="{{ $purchase_line->id }}">
                        <i class="fa fa-cogs"></i> @lang('Manage Serial')
                    </button>
                @endif
            </td>
            ...
            <td>
                ...
                required
                @if($purchase_line->product->item_type == 'imei' || $purchase_line->product->item_type == 'serial') readonly @endif
            ...
            @if(session('business.enable_lot_number') && $purchase_line->product->item_type != 'imei' && $purchase_line->product->item_type != 'serial')
                <td>
