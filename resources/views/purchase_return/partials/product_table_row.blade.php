...
<tr class="product_row">
    ...
    @if(session('business.enable_lot_number'))
        <td>
            ...
            <select name="products[{{$row_index}}][lot_number]"  class="form-control"  required>
                <option value="">Lot &amp; Expiry</option>
                @foreach($product->lot_numbers as $key2 => $lot_number)
                    <option {{ $lot_number["selected"] == true ? "selected" : ""}} value="{{$lot_number['lot_number']}}">{{$lot_number['lot_number']}}</option>
                @endforeach
            </select>
