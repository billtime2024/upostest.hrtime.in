      ...
      <div class="row">
        <div class="col-sm-12">
          ...
          <tbody>
              ...
              // dd($purchase_line);
                    $line_total =(float) $purchase_line->purchase_price * $purchase_line->quantity_returned;
                    $price = (float) $purchase_line->purchase_price_inc_tax;
                    $tax_rates = \App\TaxRate::where("id", $purchase_line->tax_id)->get();
                    if(count($tax_rates) > 0) {
                      $total_tax += ((float) $tax_rates[0]["amount"] /100 ) * $line_total;
                    }
               }
               //dd($total_tax);
              @endphp
            ...
            <td class="text-right">
                ...
                @else
                    @php
                        ...
                        $taxes = \App\TaxRate::where("id", $purchase_line["tax_id"])->get();
                        $tax_amount = 0;
                        if(count($taxes) > 0) {
                          $tax_amount = ((int) $taxes[0]["amount"] / 100 ) * $total_before_tax;
                        }
                        $final_amount = $total_before_tax + $tax_amount;
