...
@section('javascript')
    <script type="text/javascript">
        ...
        const submitOtpButton = document.getElementById("submit_button");
        ...
        mobile.addEventListener("change", (e) => {
            ...
            if(isNaN(parseInt(e.target.value))){
                toastr.error("Mobile number must be digits only");
                e.target.value = "";
            }
            
            if(e.target.value !== "" && (e.target.value[0] !== "9" || e.target.value[1] !== "1")) {
                e.target.value = "91" + e.target.value;
            }
            
            if(e.target.value[0] !== "9" || e.target.value[1] !== "1" || e.target.value.length != 12) {
                toastr.error("Please enter a valid mboile number");
            }
            ...
            try{
                //  const mobile = document.getElementById("mobile");
                 if(mobile.value.length > 0){
                    const res = await fetch("/api/send-otp/" + mobile.value + "/");
                    const data = await res.json();
                    if(data.msg === "sent"){
                       const userOtp =  prompt("Enter your 🪀 WhatsApp Otp");
                       
                       if(userOtp == null || userOtp.length < 5 ){
                            toastr.error("Invalid OTP. Try again");
                               ...
                               body: JSON.stringify({"opt": userOtp, "mobile": mobile.value})
                           })
                           const dataVerify = await resVerify.json();
                           
                           if(dataVerify.msg == "valid otp" ){
                               isOTPVerified = true;
                               toastr.success("OTP is verified")
