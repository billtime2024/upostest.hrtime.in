    ...
    <script type="text/javascript">
        $(document).ready(function() {
            profit_by_products_table = $('#profit_by_products_table').DataTable({
                ...
                footerCallback: function(row, data, start, end, display) {
                    ...
                    for (var r in data) {
                        ...
                        const single_unit =  data[r].total_unit_sold.split(" ");
                        
                        if(current_unit == "") {
                            current_unit = single_unit[1];
                            
                            total_quanity = parseFloat(single_unit[0]);
                            units.push(current_unit);
                        } else if (current_unit != single_unit[1]) {
                       
                            units.push(current_unit);
                            
                            total_quantity_per_unit.push([ current_unit,  total_quanity]);
                                 current_unit = single_unit[1];
                            total_quanity = parseFloat(single_unit[0]);
                        } else {
                             total_quanity += parseFloat(single_unit[0]);
                    ...
                    $('#profit_by_products_table .footer_total').html(__currency_trans_from_en(
                        total_profit));
                        const new_total_quantity = [];

                      if(total_quantity_per_unit.length > 1) {  
                            for(let i = 0; i < total_quantity_per_unit.length; i++) {
                                if(new_total_quantity[total_quantity_per_unit[i][0]] == undefined){
                                    new_total_quantity[total_quantity_per_unit[i][0]]  = total_quantity_per_unit[i][1];
                                } else {
                                     new_total_quantity[total_quantity_per_unit[i][0]]  += total_quantity_per_unit[i][1];
                                }
                            }
                            
                            const result = Object.keys(new_total_quantity).map((key) => [key, new_total_quantity[key]]);
                            let new_result = "";
                            
                            for(let k = 0; k < result.length; k++) {
                                new_result += result[k].reverse().join(" ");
                                if(k < result.length - 1) {
                                    new_result += "</br> ";
                                }
                            }
                            $("#profit_by_products_table .footer_total_unit").html(new_result);
                      }else {
                        $("#profit_by_products_table .footer_total_unit").html(total_quantity_per_unit.join(" ").split(",").reverse().join(" "));
                      }
                    
                    console.log( new_total_quantity);
            ...
            $('a[data-toggle="tab"]').on('shown.bs.tab', function(e) {
                ...
                //     $('#profit_by_categories_table .footer_total').html(__currency_trans_from_en(
                //         total_profit));
                //         const new_total_quantity = [];
                ...
                //                 }
                //             }
                //             $("#profit_by_categories_table .footer_total_unit").html(new_result);
                //       }else {
                //         $("#profit_by_categories_table .footer_total_unit").html(total_quantity_per_unit.join(" ").split(",").reverse().join(" "));
                //       }
                ...
                //     $('#profit_by_brands_table .footer_total').html(__currency_trans_from_en(
                //         total_profit));
                //         const new_total_quantity = [];
                ...
                //                 }
                //             }
                //             $("#profit_by_brands_table .footer_total_unit").html(new_result);
                //       }else {
                //         $("#profit_by_brands_table .footer_total_unit").html(total_quantity_per_unit.join(" ").split(",").reverse().join(" "));
                //       }
