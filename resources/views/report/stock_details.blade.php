			...
			<table class="table table-condensed bg-gray">
				...
				@foreach( $product_details as $details )
					<tr>
						...
						<td>{{ $details->business_location }}</td>
						...
						<td>
							@if($details->stock)
								<span class="display_currency" data-currency_symbol=true>{{ (float)$details->stock * $details->sell_price_inc_tax }}</span>
							@else
							 0
							@endif
						</td>
...
@section("javascript")
<script>
    document.getElementById("repair_model_id").style.display = "none";
</script>
@endsection