...
<style>
    .blink-notification {
        animation: blink_notification 700ms ease-in infinite;
          animation-iteration-count: 7;
        transition: all 500ms ease-in;
    }
    @keyframes blink_notification {
        0% {
            opacity: 0;
        }
        100% {
    ...
    <div class="tw-px-5 tw-py-3">
        <div class="tw-flex tw-items-start tw-justify-between tw-gap-6 lg:tw-items-center">
            <div class="tw-flex tw-items-center tw-gap-3">
                ...
                @php
                    $active = Modules\Superadmin\Entities\Subscription::active_subscription(auth()->user()->business_id);
                    
                @endphp
                @if ($active !== null)
                  @if((int) explode( " ", \Carbon::today()->diffInDays($active->end_date))[0]  < 30 )
                  <div style="display: flex; justify-content: center; align-items: center;  transform: translateX(-7vw);">
                 <span class="tw-bg-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-800 hover:tw-bg-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-900" style="color: #fff; font-weight: bold; padding: 0.5rem; border-radius: 0.25rem;">Your Plan Ends @ </span>
                 <span @if((int) explode( " ", \Carbon::today()->diffInDays($active->end_date))[0]  < 4 ) class="blink-notification" @endif style="background-color: #f00; color: #fff; font-weight: bold; padding: 0.3rem; border-radius: 0.25rem;" >{{ @format_date($active->end_date) }} ({{\Carbon::today()->diffInDays($active->end_date)}} @lang('lang_v1.days')) </span>
                  </div>
                 @endif
                @else 
                    <div style="display: flex; justify-content: center; align-items: center;  transform: translateX(-7vw);">
                        <span style="background-color: #f00; color: #fff; font-weight: bold; padding: 0.3rem; border-radius: 0.25rem; margin-right: 10px" > Plan Expired</span>
                         <a @if((int) explode( " ", \Carbon::today()->diffInDays($active->end_date))[0]  < 4 ) class="blink-notification" @endif  href="{{action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'index'])}}"><button style="color: #fff;background: linear-gradient(85deg, #0a9264 20%, #10B981 30%, #09885d 40%); font-weight: bold; padding: 0.4rem; margin-left: 10px; border-radius: 0.25rem;">Choose New Plan</button></a>
                    </div>
                @endif
