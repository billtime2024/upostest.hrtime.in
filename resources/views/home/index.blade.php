...
@php
//dd(auth()->user()->can('dashboard.data'), $is_admin, $all_locations);
@endphp
            ...
            <div class="tw-grid tw-grid-cols-1 tw-gap-4 sm:tw-gap-5 lg:tw-grid-cols-2">
                @if (auth()->user()->can('sell.view') || auth()->user()->can('direct_sell.view'))
                    @if (!empty($all_locations))
                        <div
                            class="tw-transition-all lg:tw-col-span-2 xl:tw-col-span-2 tw-duration-200 tw-bg-white tw-shadow-sm tw-rounded-xl tw-ring-1 hover:tw-shadow-md hover:tw--translate-y-0.5 tw-ring-gray-200" style="display: flex; justify-content: center;">
                            <div class="tw-p-4 sm:tw-p-5" style="width: 70% !important;">
                             ...
                             <div style="width: 30% !important; height: 50% !important; margin-top: 1rem;">
                                   <div class="tw-flex tw-items-center tw-gap-2.5">
                                    <div
                                        class="tw-border-2 tw-flex tw-items-center tw-justify-center tw-rounded-full tw-w-10 tw-h-10">
                                        <svg aria-hidden="true" class="tw-size-5 tw-text-sky-500 tw-shrink-0"
                                   ...
                                   <div id="sell_donut_pi_chart_monthly" class="sell_donut_pi_chart_monthly"></div>
                             </div>
                        </div>
                    ...
                    @if (!empty($all_locations))
                        <div
                            ...
                            <div style="width: 30% !important; margin-top: 1rem; ">
                                                                <div class="tw-flex tw-items-center tw-gap-2.5">
                                    <div
                                        class="tw-border-2 tw-flex tw-items-center tw-justify-center tw-rounded-full tw-w-10 tw-h-10">
                                        <svg aria-hidden="true" class="tw-size-5 tw-text-sky-500 tw-shrink-0"
                                ...
                                <div id="sell_donut_pi_chart_yearly" class="sell_donut_pi_chart_yearly"></div>
                            </div>
                        </div>
                    @endif
                @endif
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        @if (isset($sells_chart_1))
        let total_monthy = 0;
        const datasets = {!! json_encode($sells_chart_1->datasets) !!};
        const newDataset = [];
        datasets.forEach((dataset, index) => {
            if(index < datasets.length -1 ){
                ...
                newDataset.push({x: dataset.name, y: total, name: dataset.name});
            }
        })
        newDataset.forEach(dataset => {
            const percentage = (parseFloat(dataset.y) / parseFloat(total_monthy)) * 100;
            dataset.percentage = percentage;
        })
        Highcharts.chart('sell_donut_pi_chart_monthly', {
    chart: {
       type: 'pie',
        zooming: {
            type: 'xy'
...
credits: {
    ...
    // title: { text: "<h3 class='tw-font-bold tw-text-base lg:tw-text-xl'> {{ __('home.sells_last_30_days') }}</h3>"},
      title: { text: ""},
    plotOptions: {
        series: {
  ...
  tooltip: {
        enabled: true,
        valuePrefix: ' {point.percentage:.0f}% (',
        valueSuffix: ') ',
    },
    series: [{ 
            name: 'Monthly',
    ...
    @endif
    @if(isset($sells_chart_2))
    let total_year = 0;
    const datasets_year = {!! json_encode($sells_chart_2->datasets) !!};
        const newDataset_1 = [];
        datasets_year.forEach((dataset, index) => {
            if(index < datasets_year.length -1 ){
                let total = 0;
                dataset.values.forEach(value => {
                    total += value;
                })
                ...
                dataset.y = total;
                dataset.x = dataset.name;
                newDataset_1.push({x: dataset.name, y: total, name: dataset.name});
            }
            
        })
        
        newDataset_1.forEach(dataset => {
            const percentage = (parseFloat(dataset.y) / parseFloat(total_year)) * 100;
            dataset.percentage = parseFloat(percentage.toFixed(2));
        })
        Highcharts.chart('sell_donut_pi_chart_yearly', {
    chart: {
       type: 'pie',
        zooming: {
            type: 'xy'
...
credits: {
    ...
    // title: { text: "<h3 class='tw-font-bold tw-text-base lg:tw-text-xl'>{{ __('home.sells_current_fy') }}</h3>"},
    title: {text: "" },
