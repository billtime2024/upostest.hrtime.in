...
{!! Form::open(['url' => action([\App\Http\Controllers\ProductController::class, 'update'] , [$product->id] ), 'method' => 'PUT', 'id' => 'product_add_form',
        ...
        @if(session('business.enable_product_expiry'))
          ...
          <div class="col-sm-4">
              <div class="form-group">
                  ...
                  {!! Form::select('item_type', ['none' => 'None', 'imei' => 'IMEI', 'serial' => 'Serial'], old('item_type', $product->item_type ?? 'none'), ['class' => 'form-control']) !!}
              </div>
          </div>
    ...
    @component('components.widget', ['class' => 'box-primary'])
        <div class="row">
            ...
            <div class="col-sm-4">
                <div class="form-group">
                    {!! Form::label('enable_reward_points', __('product.enable_reward_points') . ':') !!}
                    {!! Form::checkbox('enable_reward_points', 1, $product->enable_reward_points, ['class' => 'input-icheck', 'id' => 'enable_reward_points']); !!}
                </div>
            </div>
            <div class="col-sm-4" id="reward_points_per_amount_div" @if(!$product->enable_reward_points) style="display: none;" @endif>
                <div class="form-group">
                    {!! Form::label('reward_points_per_amount', __('product.reward_points_per_amount') . ':') !!}
                    {!! Form::number('reward_points_per_amount', $product->reward_points_per_amount ? @num_format($product->reward_points_per_amount) : null, ['class' => 'form-control input_number', 'placeholder' => __('product.amount_for_1_point'), 'min' => '0', 'step' => '0.01']); !!}
                </div>
            </div>
  ...
  <script src="{{ asset('js/product.js?v=' . $asset_v) }}"></script>
    <script type="text/javascript">
      $(document).ready( function(){
        ...
        $('#enable_reward_points').on('ifChecked', function(){
          $('#reward_points_per_amount_div').show();
        });
        $('#enable_reward_points').on('ifUnchecked', function(){
          $('#reward_points_per_amount_div').hide();
          $('#reward_points_per_amount').val('');
        });
      });
    </script>
@endsection