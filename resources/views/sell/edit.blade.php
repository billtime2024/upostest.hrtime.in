			...
			@component('components.widget', ['class' => 'box-solid'])
				...
				<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
					<div class="form-group">
						...
						<div class="input-group">
							...
							{!! Form::text('transaction_date', $transaction->transaction_date, ['class' => 'form-control', 'id' => 'date', 'readonly', 'required']); !!}
						</div>
					</div>
				</div>
				@php
				...
				@else
					<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
						<div class="form-group">
							...
							{!! Form::select('status', $statuses, $status, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'id' => 'status', 'required']); !!}
						</div>
					</div>
				@endif
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						{!! Form::select('invoice_scheme_id', $invoice_schemes, $default_invoice_schemes->id, ['class' => 'form-control select2','id' => 'invoice_scheme','placeholder' => __('messages.please_select')]); !!}
					</div>
				</div>
				@endif
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						{!! Form::text('invoice_no', $transaction->invoice_no, ['class' => 'form-control', 'id' => 'invoice_no', 'placeholder' => $transaction->type == 'sales_order' ? __('restaurant.order_no'): __('sale.invoice_no')]); !!}
					</div>
				</div>
				@endcan
				@php
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $(document).on('change', '.imei_serial_select', function() {
            var selectedOptions = $(this).val();
            var rowIndex = $(this).data('row-index');
            var quantityField = $('input[name="products[' + rowIndex + '][quantity]"]');

            if (selectedOptions && selectedOptions.length > 0) {
                quantityField.val(selectedOptions.length);
            } else {
                quantityField.val(1);
            }
        ...
        // Load IMEI/Serial numbers when product row is added
        function loadImeiSerialNumbers(row) {
            var imeiSerialSelect = row.find('.imei_serial_select');
            if (imeiSerialSelect.length > 0) {
                var variationId = imeiSerialSelect.data('variation-id');
                var locationId = $('input#location_id').val();

                $.ajax({
                    url: '/sell/get-available-imei-serial-numbers',
                    method: 'GET',
                    data: {
                    ...
                    success: function(response) {
                        if (response.length > 0) {
                            var itemType = row.find('input[name*="item_type"]').val() || 'imei'; // fallback
                            var label = itemType == 'imei' ? 'IMEI' : 'Serial';
                            var selectText = '{{ __("lang_v1.select") }}';
                            var numbersText = '{{ __("lang_v1.numbers") }}';
                            var options = '<option value="">' + selectText + ' ' + label + ' ' + numbersText + '</option>';
                            response.forEach(function(item) {
                                options += '<option value="' + item.identifier + '">' + item.identifier + '</option>';
                            });
                            imeiSerialSelect.html(options);
                            // Initialize select2 if needed
                            if (imeiSerialSelect.hasClass('select2')) {
                                imeiSerialSelect.select2();
                            }
                        }
                    }
                });
        ...
        // Handle IMEI/Serial button click
        $(document).on('click', '.select-imei-serial-btn', function() {
            var rowIndex = $(this).data('row-index');
            var variationId = $(this).data('variation-id');
            var itemType = $(this).data('item-type');
            var locationId = $('input#location_id').val();
            var sellLineId = $('input[name="products[' + rowIndex + '][transaction_sell_lines_id]"]').val();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var selectedIdentifiers = selected.map(function(item) { return item.identifier; });
                    if (available.length > 0) {
                        available.forEach(function(item) {
                            var checked = selectedIdentifiers.includes(item.identifier) ? 'checked' : '';
                            listHtml += '<div class="checkbox"><label><input type="checkbox" class="imei-serial-checkbox" value="' + item.identifier + '" ' + checked + '> ' + item.identifier + '</label></div>';
                        });
                    } else {
        ...
        // Override pos_product_row to load IMEI/serial after row is added
        ...
        window.pos_product_row = function(variation_id, purchase_line_id, weighing_scale_barcode, quantity) {
            ...
            // Load IMEI/serial for the last added row
            setTimeout(function() {
                var lastRow = $('table#pos_table tbody tr').last();
                loadImeiSerialNumbers(lastRow);
            }, 100);
        };

        const statusEle = document.getElementById("status");

        if(statusEle.value === "quotation") {
            $('#status').on('change', function(e) {
                if(e.target.value === "final"){
                    ...
                    if(document.getElementById("date")){
                        const dateTime = __current_datetime();


                        document.getElementById("date").value = dateTime;
                    }
                    const businessLocations =  {!! \App\BusinessLocation::where('business_id', auth()->user()->business_id)->get() !!};
                    const quotationLocation = {!! $transaction->location_id !!};
                    const permittedLocation = "{!! auth()->user()->permitted_locations()  !!}";

                    if(permittedLocation === "all") {
                        businessLocations.forEach(location => {
                            if(quotationLocation === location.id) {
                                $("#invoice_scheme").val(location.invoice_scheme_id);
                                if(document.getElementById("invoice_scheme")) {
                                    document.getElementById("invoice_scheme").value = location.invoice_scheme_id;
                                }
                                $("#invoice_scheme").trigger('change');
                            }
                        })
                    }
                }
