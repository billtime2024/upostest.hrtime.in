...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.select-imei-serial-btn', function() {
            ...
            var modal = $('#imei-serial-modal-' + rowIndex);
        ...
        // Conditional logic for lot and IMEI/serial selection
        $(document).on('change', '.lot-imei-selector[data-selector-type="lot"]', function() {
            var rowIndex = $(this).data('row-index');
            var selectedLot = $(this).val();
            var imeiBtn = $('button.lot-imei-selector[data-selector-type="imei"][data-row-index="' + rowIndex + '"]');

            if (selectedLot) {
                // Disable IMEI button if lot is selected
                imeiBtn.prop('disabled', true).addClass('disabled');
                // Clear any selected IMEI/serial
                $('input[name="products[' + rowIndex + '][selected_imei_serial][]"]').remove();
                // Reset quantity to 1 if it was set by IMEI selection
                var quantityField = $('input[name="products[' + rowIndex + '][quantity]"]');
                ...
                // Enable IMEI button if no lot is selected
                imeiBtn.prop('disabled', false).removeClass('disabled');
            }
        });

        $(document).on('click', '.lot-imei-selector[data-selector-type="imei"]', function() {
            var rowIndex = $(this).data('row-index');
            var lotSelect = $('select.lot-imei-selector[data-selector-type="lot"][data-row-index="' + rowIndex + '"]');

            // Disable lot select when IMEI is selected
            lotSelect.prop('disabled', true).addClass('disabled');
            // Clear lot selection
            lotSelect.val('');
            lotSelect.trigger('change');
        });

        // Re-enable lot select when IMEI modal is closed without selection
        $(document).on('hidden.bs.modal', '#imei-serial-modal', function() {
            var rowIndex = $(this).data('row-index');
            var selectedCount = $(this).find('.imei-serial-checkbox:checked').length;

            if (selectedCount === 0) {
                // No IMEI selected, re-enable lot select
                var lotSelect = $('select.lot-imei-selector[data-selector-type="lot"][data-row-index="' + rowIndex + '"]');
                lotSelect.prop('disabled', false).removeClass('disabled');
            }
        });
