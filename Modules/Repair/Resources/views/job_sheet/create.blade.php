...
<section class="content">
    ...
    {!! Form::open(['action' => '\Modules\Repair\Http\Controllers\JobSheetController@store', 'id' => 'job_sheet_form', 'method' => 'post', 'files' => true]) !!}
        ...
        <div class="row">
			...
			@endif
            @php
                $new_locations = \App\BusinessLocation::forDropdown(auth()->user()->business_id, false, true);
                if($default_location == null){
                   // $default_location = \App\BusinessLocation::where("business_id", auth()->user()->business_id)->get()[0]->id;
                }
                $new_default_location = \App\BusinessLocation::find($default_location);
            @endphp
            <div class="col-md-3">
                <div class="form-group">
                    {!! Form::label('location_id', __('business.business_location') . ':*' )!!}
                    	{!! Form::select('location_id',  $business_locations, old('location_id') ? old('location_id') : $default_location, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required'], $new_locations['attributes']); !!}
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-inline">
                        <label class="cursor-pointer">
                            <input type="checkbox" name="send_notification[]" value="sms" checked="checked">
...
@section('javascript')
    ...
    <script type="text/javascript">
    	...
    	$(document).ready( function() {
    	    $(document).ready( function(){
      		if(document.querySelector("#location_id").selectedIndex == 0){
			...
			$('select#location_id').change(function() {
			  ...
			  if($("select#location_id").val() == ""){
              		    const payment_types_dropdown = $('.payment_types_dropdown');
            			const payment_type = payment_types_dropdown.val();
            			const payment_row = payment_types_dropdown.closest('.payment_row');
        		        const account_dropdown = payment_row.find('select#account_0');
		              account_dropdown.attr('disabled', 'true');
		              $(".payment_types_dropdown").attr("disabled", "true");
		          }
    	});
		    ...
		    $(document).on('change', '#method_0, #location_id', function(e) {
		         if(e.target.id == 'location_id' && e.target.id !== 'method_0'){
		          
		          $(".payment_types_dropdown").val("none");
    			...
    			const payment_row = payment_types_dropdown.closest('.payment_row');
		        const account_dropdown = payment_row.find('select#account_0');
		        account_dropdown.val(null);
		        account_dropdown.change();
		        if($("select#location_id").val() == ""){
		              account_dropdown.attr('disabled', 'true');
		              $(".payment_types_dropdown").attr("disabled", "true");
		          } else {
		              account_dropdown.removeAttr('disabled');
		              $(".payment_types_dropdown").removeAttr("disabled");
		          }
			    }else {
	        ...
	        var row_index = payment_row.find('.payment_row_index').val();
	            var account_dropdown = payment_row.find('select#account_0');
		    if (payment_type && payment_type != 'advance') {
			    ...
			    if (payment_type) {
			        var default_account = default_accounts && default_accounts[payment_type] && default_accounts[payment_type]['account'] ? 
			            default_accounts[payment_type]['account'] : '';
			        var account_dropdown = $('select#account_0');
			        if (account_dropdown.length && default_accounts) {
			            account_dropdown.val(default_account);
			            account_dropdown.change();
			        }
