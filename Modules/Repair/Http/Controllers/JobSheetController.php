...
class JobSheetController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $job_sheets = JobSheet::with('invoices')
                    ...
                    // ->leftJoin('transactions as t', 'repair_job_sheets.id', '=', 't.repair_job_sheet_id')
                    ->leftJoin('transactions as t', function ($query){
                        $query->on('repair_job_sheets.id', '=', 't.repair_job_sheet_id')
                                ->whereRaw('t.type = "repair"');
                    })
                    ->leftJoin('transaction_payments as tp', 't.id', '=', 'tp.transaction_id')
                    ->where('repair_job_sheets.business_id', $business_id)
                    ->select('repair_job_sheets.delivery_date', 'job_sheet_no', DB::raw("CONCAT(COALESCE(technecian.surname, ''),' ',COALESCE(technecian.first_name, ''),' ',COALESCE(technecian.last_name,'')) as technecian"), DB::raw("CONCAT(COALESCE(users.surname, ''),' ',COALESCE(users.first_name, ''),' ',COALESCE(users.last_name,'')) as added_by"), 'contacts.name as customer','contacts.mobile as phone' ,'b.name as brand', 'rdm.name as device_model', 'serial_no', 'estimated_cost', 'rs.name as status', 'repair_job_sheets.id as id', 'repair_job_sheets.created_at as created_at', 'service_type', 'rs.color as status_color', 'bl.name as location', 'rs.is_completed_status', 'device.name as device', 'repair_job_sheets.custom_field_1', 'repair_job_sheets.custom_field_2', 'repair_job_sheets.custom_field_3', 'repair_job_sheets.custom_field_4', 'repair_job_sheets.custom_field_5', 'tp.amount as payment_amount', 'tp.method as payment_method');
            ...
            if (!empty(request()->date_range)) {
                ...
                if (count($date_parts) == 2) {
                    $start_date = Carbon::createFromFormat(session('business.date_format'), trim($date_parts[0]))->startOfDay();
                    $end_date = Carbon::createFromFormat(session('business.date_format'), trim($date_parts[1]))->endOfDay();
    ...
    public function create()
    {
        ...
        $default_status = '';
        $dummyPaymentLine = ['method' => '', 'amount' => 0, 'note' => '', 'card_transaction_number' => '', 'card_number' => '', 'card_type' => '', 'card_holder_name' => '', 'card_month' => '', 'card_year' => '', 'card_security' => '', 'cheque_number' => '', 'bank_account_number' => '',
        ...
        $payment_lines[] = $dummyPaymentLine;
        $payment_line = $dummyPaymentLine;
             //Accounts
        $accounts = [];
        ...
        return view('repair::job_sheet.create')
            ->with(compact('repair_statuses', 'device_models', 'brands', 'devices', 'default_status', 'technecians', 'business_locations', 'types', 'customer_groups', 'walk_in_customer', 'repair_settings', 'change_return', 'payment_types', 'payment_lines', 'payment_line', 'accounts'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
         if($request->payment[0]["method"] != "none"){
            ...
            $transaction->created_by = $request->user()->id !== NULL ? $request->user()->id : auth()->user()->id;
            $transaction->repair_job_sheet_id = $job_sheet->id;
            $transaction->save();
            
            $payment["transaction_id"] = (string) $transaction->id;

            $new_request = new \Illuminate\Http\Request();
            $new_request->replace($payment);
            ...
            $transactionUtil = new \App\Utils\TransactionUtil;
            $tp = $transactionUtil->payContact($new_request);
            ...
            $transaction = $tp;
            $transaction->contact = Contact::find($new_request->input('contact_id'));
            ...
            $payment["transaction_type"] = "sell";
            $new_payments = [];
            $new_payments[0] = $payment;
            $cashRegisterUtil->addSellPayments($transaction, $new_payments);
            }          
            // $this->cashRegisterUtil->updateSellPayments($status_before, $transaction, $new_request->payment);
        //   $cashRegisterUtil->updateSellPayments($transaction->status, $transaction, $new_payments);
        ...
        // }
            ...
            // $transaction_payments = new \App\TransactionPayment();
            // $transaction_payments->transaction_id = $transaction->id;
            // $transaction_payments->business_id = $business_id;
            // $transaction_payments->amount = $payment["amount"];
            // $transaction_payments->method = $payment["method"];
            // $transaction_payments->created_by = $request->user()->id !== NULL ? $request->user()->id : auth()->user()->id;
            // $transaction_payments->payment_type = "credit";
            // $transaction_payments->is_advance = 1;
            // $transaction_payments->paid_on = $payment["paid_on"]  !==  NULL ? $payment['paid_on']: \Carbon\Carbon::now();
            // $transaction_payments->card_number = $payment["card_number"];
            // $transaction_payments->card_holder_name = $payment["card_holder_name"];
            // $transaction_payments->card_transaction_number = $payment["card_transaction_number"];
            // $transaction_payments->payment_for = $request->contact_id;
            // $transaction_payments->card_type = $payment["card_type"];
            // $transaction_payments->card_month = $payment["card_month"];
            // $transaction_payments->account_id = $payment["account_id"];
            // $transaction_payments->card_year = $payment["card_year"];
            // $transaction_payments->card_security = $payment["card_security"];
            // $transaction_payments->cheque_number = $payment["cheque_number"];
            // $transaction_payments->bank_account_number = $payment["bank_account_number"];
            // $transaction_payments->note = $payment["note"];
