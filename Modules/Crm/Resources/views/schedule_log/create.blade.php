        ...
        {!! Form::open(['url' => action([\Modules\Crm\Http\Controllers\ScheduleLogController::class, 'store']), 'method' => 'post', 'id' => 'schedule_log_form' ]) !!}
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-md-4">
                       <div class="form-group">
                            {!! Form::label('status', 'Follow Up Status' .':') !!}
                            {!! Form::select('status', $statuses, $schedule->status, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'style' => 'width: 100%;', 'id' => 'follow_up_status']); !!}
                       </div>
                    </div>
                    <div id="next_follow_up_container" @if($schedule->status !== "scheduled") style="display: none;" @endif class="col-md-4">
                       <div class="form-group">
                            {!! Form::label('next_follow_up', 'Next Follow up' .':') !!}
                            {!! Form::text('next_follow_up', null, ['class' => 'form-control datetimepicker', 'required', 'readonly', 'id' => 'next_follow_up']) !!}
                       </div>
                    </div>
                </div>
            </div>
...
<script>
    // $("#next_follow_up_container").hide();
    
    $("#follow_up_status").on("change", function (e) {
        if(e.target.value === "scheduled"){
            $("#next_follow_up_container").show();
        } else {
            $("#next_follow_up_container").hide();
        }
    })
