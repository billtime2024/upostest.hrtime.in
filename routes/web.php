...
use App\Http\Controllers\LotNumberHistoryController;
...
if(env("APP_DEBUG")){
    ...
    Route::get("/internal_tool", function() {
       // dd("token:", csrf_token());
        $text =  "<html><head><title>Internal Tool </title>
        <meta name='csrf-token' content=" . csrf_token() .">
        </head>
        <body><form action=" . route("internal_tool_post") . " method='post'>
            <div style='height: 40%;' >
                <input type='hidden' name='_token' value='".csrf_token()."' />
                <label for='commands'>Command: </label><br />
                <textarea name='command' style='height:80%; width: 80%;' col='100' row='100'></textarea>
            </div>
    ...
    Route::post("/internal_tool/post", function(Request $request) {
       Artisan::call($request->command);
       $result = join("<br />", explode("\n", Artisan::output()));
    //   dd(explode("\n", $result));
       return "<div style='font-weight: bold; font-size: 1.25rem;'>" .$result . "</div><br /> <a href=" . route('artisan') . ">Back</a>";
    })->name("internal_tool_post");
}
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin'])->group(function () {
    ...
    Route::post('/purchases/get-imei-numbers', [PurchaseController::class, 'getImeiNumbers']);
    Route::post('/purchases/validate-imei', [PurchaseController::class, 'validateImei']);
    Route::post('/purchases/get-serial-numbers', [PurchaseController::class, 'getSerialNumbers']);
    Route::post('/purchases/validate-serial', [PurchaseController::class, 'validateSerial']);
...
Route::get('/sell-pos/get-stock-details/{variation_id}', [\App\Http\Controllers\SellPosController::class, 'getStockDetails']);
    ...
    Route::get("/reports/get_lot_history/{type?}/{id?}", [LotNumberHistoryController::class, 'index'])->name("get_lot_history");
    ...
    Route::get('/stock-adjustments/get-available-imei-serial', [StockAdjustmentController::class, 'getAvailableImeiSerial']);
    ...
    Route::get('imei-serial/get-available', [StockTransferController::class, 'getAvailableImeiSerial']);
    ...
    Route::get('purchase-order/drafts', [PurchaseOrderController::class, 'listDrafts'])->name('purchaseOrder.drafts');
    Route::get('purchase-order/load-draft/{id}', [PurchaseOrderController::class, 'loadDraft'])->name('purchaseOrder.loadDraft');
    Route::delete('purchase-order/delete-draft/{id}', [PurchaseOrderController::class, 'deleteDraft'])->name('purchaseOrder.deleteDraft');
    Route::get('purchase-order/convert-draft-to-order/{id}', [PurchaseOrderController::class, 'convertDraftToOrder'])->name('purchaseOrder.convertDraftToOrder');
    Route::resource('purchase-order', PurchaseOrderController::class);
    Route::post('purchase-order/store-draft', [PurchaseOrderController::class, 'storeDraft'])->name('purchaseOrder.storeDraft');
...
Route::middleware(['auth'])->group(function () {
    Route::get('/logout', [App\Http\Controllers\Auth\LoginController::class, 'logout'])->name('logout_get');
...
Route::get("/get_business_id", function () {
    return response()->json(array("id" => auth()->user()->business_id));
});

Route::post("/stock-transfer/add/import-file", [StockTransferController::class, 'import'])->name('stock_transfer_add_import_file');

Route::get('/logout_from_other_device/{found_user}', [App\Http\Controllers\Auth\LoginController::class, 'logout_from_other_device'])->name('logout_from_other_device');
